...
- **WalletConnect Verify:** پروپ reachability با no-cors (پایان false
  ...
  برمی‌گردد.
- **assetlinks.json (۴۰۴ زنده — اندازه‌گیری‌شده):** قدم `ci/assetlinks-publish.sh` در
  ورک‌فلوی Build APK فایل را از کِی‌استور امضا با keytool می‌سازد و فقط روی
  تغییر کامیت می‌کند — هرگز debug/حدسی؛ اعتبارسنجی با `--check`.
