...
- **«دکمه چپ و راست» حذف شد — ریشه یک باگ بود.** دو نقطهٔ ۵px نشانگر `<button>` بودند و قانونِ سراسریِ `button { min-height: 44px; min-width: 44px }` هر کدام را به یک قرصِ خاکستریِ ۴۴px وسطِ بنر، روی زیرعنوان، تبدیل می‌کرد. نشانگر حالا دو قطعهٔ باریکِ `<span>` با `pointer-events: none` است که قطعهٔ فعالش در همان ۱۰ ثانیهٔ توقف پر می‌شود؛ فلشِ گردِ انتهای اسلاید هم رفت. تنها دکمه‌ها خودِ دو اسلایدند؛ کیبورد با کلیدهای جهت (در RTL آینه) جابه‌جا می‌شود.
- **باکس از نو:** سطحِ شیشه‌ای از توکن‌های اپ، دو میدانِ شفق به رنگِ هر اسلاید که با تعویض در هم محو می‌شوند، دنباله‌دارِ نور روی کادر (چرخش با `transform`، نه انیمیشنِ نقاشی)، ورقِ نور، شبکهٔ نقطه‌ای، گویِ ۶۰px با دو حلقهٔ خلاف‌جهت و گلیف‌های زندهٔ ترسیم‌شده، اسپارک‌لاینِ خودکش با ماسک. ۸۶ → ۱۱۲px روی گوشی، ۹۶ → ۱۲۸px دسکتاپ. متن همان دو خط.
- **بودجهٔ حرکت:** همه با transform/opacity جز دو خط‌کشیِ SVG؛ اسلایدِ پنهان paused؛ در reduced-motion صفر انیمیشنِ زنده و فریمِ ثابتِ کامل (با Chromium شمرده شد).
- **تم و زبان:** تم روشن اکسنت‌های عمیق‌تر و شفقِ نصف می‌گیرد (و بلوکِ پیش‌از‌`data-theme` هم)، کنتراستِ عنوان/زیرعنوان روی سطحِ ترکیب‌شده در سه تم ≥ ۱۴٫۶ / ۶٫۳:۱؛ فارسی فقط برای `fa`، چیدمان از `dir` صفحه.
- **باگِ سرِ راه:** مسیرِ اسپارک‌لاین `fill="none"` نداشت و SVG با وترِ سیاه می‌بستش — یک گوهٔ تیره زیرِ خط، در تم روشن مثل سایه. بسته و در تست قفل شد.
- **تست:** probe از ۳۲ به **۶۱** بررسی (فقط یک نوع `<button>`، نشانگرِ غیرِدکمه، اندازه‌ها، ۱۶ keyframe فقط compositor، کنتراست با شفقِ زیرِ متن در سه تم)؛ سوئیتِ mount از ۶ به **۹** تستِ بنر (دو دکمه و بس، کلیدهای جهت در RTL/LTR، id یکتای گرادیان‌ها). wiring: همان ۱۴ شکستِ پیشینِ main. `vite build` سبز.

# ۲۰۲۶-۰۹-۲۴ (۵) — چشمکِ لندینگ ریشه‌ای بسته شد (۴۵ → ۸ انیمیشن، ۱٫۱ → ۵۷ فریم)؛ شش صفحهٔ سئوی تازه در سه جفتِ دوزبانه؛ ۱۷ شبکه در همه‌جا یک‌سان شد؛ llms.txt و HowTo

## وبلاگ: هشت صفحهٔ تازه، شش راهنمای بلند و دو هاب (۲۰۲۶-۰۹-۲۴ (۵)، ادامه)

سه جفتِ راهنمای بلند اضافه شد — نه خلاصه، بلکه همان استدلال در دو زبان:

| انگلیسی | فارسی | موضوع |
|---|---|---|
| `/how-crypto-swap-fees-work` | `/کارمزد-سواپ-ارز-دیجیتال` | کارمزد از چه چیزی ساخته شده (شبکه، استخر، پلتفرم) |
| `/custodial-vs-non-custodial-wallets` | `/تفاوت-کیف-پول-امانی-و-غیرامانی` | امانت‌داری چه چیزی را تعیین می‌کند و غیرامانی چه چیزی را حل نمی‌کند |
| `/what-stays-private-without-kyc` | `/بدون-احراز-هویت-چه-چیزی-خصوصی-می-ماند` | چه چیزی جمع نمی‌شود و زنجیره/RPC چه چیزی را می‌بیند |

به‌علاوه `/blog` و `/وبلاگ` به‌عنوان دو هاب؛ فهرستشان از خودِ `POSTS` رندر می‌شود، پس
هیچ راهنمایی نمی‌تواند بدون لینک‌شدن روی صفحهٔ فهرست باقی بماند (صفحهٔ یتیم).

- **`BlogPosting` + `Blog`** با تاریخ واقعی، ناشر و مجموعه. نویسنده **شرکت** است نه یک
  اسم ساختگی: این پروژه نویسندهٔ شخصی ندارد و بای‌لاین جعلی چیزی است که یک بازبین با
  یک کلیک پیدا می‌کند.
- **یک باگ واقعی که همین کار رو کرد:** پیش‌فرضِ متنِ ریسک و دکمهٔ CTA در `render()`
  رشتهٔ انگلیسی هاردکد بود؛ هر صفحهٔ فارسی که `riskText` خودش را نداشت، یک پاراگراف
  انگلیسی وسط صفحهٔ فارسی تحویل می‌گرفت. حالا پیش‌فرض‌ها در `copyFor()` و به زبان
  خودِ صفحه‌اند، و `qa-dist` همین را تست می‌کند.
- **بودجهٔ حرکت به قالب راهنماها هم آمد.** قالب `gen-landing` هنوز گریدِ متحرک، سه
  هالهٔ شناور، حلقهٔ چرخان، ورقِ براق و نقطهٔ تپنده را بی‌نهایت اجرا می‌کرد؛ حالا زیر
  `@media (hover: none), (max-width: 999px)` همه ثابت می‌مانند و `backdrop-filter`
  پنل‌ها برداشته می‌شود — همان درمانی که روی لندینگ دوزبانه جواب داد.
- **sitemap از ۱۴ به ۲۲ نشانی** و `submit-indexnow` با هر هشت slug تازه هم‌گام شد
  (`test/wiring.mjs` اگر یکی جا بماند قرمز می‌شود). گراف لینک `noscript` صفحهٔ اصلی هم
  از ۱۰ به ۱۸ راهنما رسید.
- **تست:** `npm run test:landing` از ۹۷ به **۱۱۴** ادعا رسید (۱۷ ادعای تازه برای فرم
  راهنماها، از جمله «راهنمای فارسی پاراگراف انگلیسی ندارد» و «هیچ راهنمایی سود یا
  رتبه وعده نمی‌دهد»)، همه سبز. `node test/wiring.mjs`: ۱۵ خطا — دقیقاً همان مجموعهٔ
  قبلی روی `main`.

جزئیات: [2026-09-24-seo-landing-pages-and-motion-budget-fa.md](2026-09-24-seo-landing-pages-and-motion-budget-fa.md).

- **«صفحه چشمک‌زن است» یک باگِ چیدمان نبود، بودجهٔ فریم بود.** با یک Chromium واقعی روی
  `dist/` شمرده شد: **۴۵ انیمیشن هم‌زمان** زنده، که ۲۰ تای آن‌ها لایهٔ تمام‌صفحه بودند
  (۶ زوم عکس، ۶ خط SVG، ۸ قرص LIVE، ۵ نوار متحرک، ستاره‌ها و شهاب‌ها). نتیجه: **۱٫۱ فریم
  در ثانیه**. در فارسی بدتر بود چون متن بلندتر است و صفحه بلندتر، پس لایه‌های بیشتری
  هم‌زمان زنده‌اند. حالا با یک بخش «MOTION BUDGET» در `scripts/landing-v2/styles.mjs`
  که با `@media (hover: none), (max-width: 999px)` کار می‌کند (بدون هیچ تشخیص
  user-agent): **۸ انیمیشن، ۵۷٫۴ فریم**. دسکتاپ دست‌نخورده.
- **سه هزینه‌ای که حذف شد:** `filter: blur(90px)` روی سه هالهٔ تمام‌صفحه (گرادیان شعاعی
  خودش محوِ نرم است)، `backdrop-filter` روی `.panel` و هدرِ چسبان در موبایل (پشت هر کارت،
  در هر فریم اسکرول)، و `pointermove` پارالاکس روی دستگاه لمسی.
- **چشمک واقعی:** کرسر تایپ‌رایتر با `steps(1)` خاموش‌روشن می‌شد — یک هرتز، بی‌نهایت.
  حالا بین ۰٫۸۵ و ۰٫۲۵ نفس می‌کشد و زیر `prefers-reduced-motion` می‌ایستد.
- **یک دروغ کوچک برداشته شد:** `markUpdated()` مهر «به‌روزرسانی ۲۱:۰۶» را حتی وقتی هر پنج
  فید شکست خورده بودند و همهٔ اعداد «—» بودند می‌نوشت. حالا فقط با جواب‌دادن `global` یا
  `markets` نوشته می‌شود؛ وگرنه «داده در دسترس نیست» می‌ماند.
- **دو خط رزرو برای خط توضیح بازار** (۰٫۰۵۶ شیفت چیدمان از رشد یک خط می‌آمد) و
  **شبکهٔ اطمینانِ ۲٫۵ ثانیه‌ای** برای `.reveal` اگر اسکریپت نیمه‌کاره بماند.
- **شش صفحهٔ سئوی تازه، سه جفتِ واقعاً ترجمه‌شده:** `/سواپ-ارز-دیجیتال` ↔
  `/crypto-swap-without-kyc`، `/سرمایه-گذاری-در-ارز-دیجیتال` ↔
  `/crypto-investing-yield-and-lending`، `/سواپ-سولانا` ↔ `/solana-token-swap` — برای
  همان سه قصدی که هیچ صفحه‌ای نداشتند («سواپ، کریپتو، سرمایه‌گذاری»). هر شش صفحه
  `FAQPage` + `HowTo` (گام‌ها روی صفحه هم رندر می‌شوند، از یک منبع) + `hreflang`
  دوطرفه دارند و sitemap از ۷ به **۱۴ نشانی** رسید. کارمزد همان ۰٫۷٪ است و هرگز بدون
  «پیش از امضا روی همان صفحه» نمی‌آید.
- **سه عدد متناقض روی شبکه‌ها یک‌سان شد.** عنوان می‌گفت ۱۷، `og` می‌گفت ۱۴، JSON-LD
  می‌گفت ۱۰ و لندینگ دوزبانه در پنج جا «۱۰ شبکه». عدد درست از رجیستری درآمد
  (`aggregatorSupports` + `openOceanSupports` + سولانا) → **۱۷**، و کارت‌های شبکهٔ
  لندینگ از ۱۰ به ۱۷ رسید (هر نام شبکه خودش یک کوئری است).
- **برای موتورهای پاسخ:** `public/llms.txt` تازه (با بخش «چه چیزی نیست»)، نام‌بردن
  صریح خزنده‌های هوش مصنوعی در `robots.txt` (بدون تغییر رفتار)، `sameAs` و
  `alternateName` فارسی در JSON-LD، و گراف لینک داخلی `<noscript>` از ۴ راهنما به ۱۰.
- **تست:** `npm run build` سبز (۱۳ صفحه)؛ `npm run test:landing` **۹۷/۰**؛
  `node test/wiring.mjs` این شاخه **۲۷۰۹ ادعا / ۱۵ خطا** و `main` **۲۷۰۸ / ۱۵** — و
  مجموعهٔ خطاها **مو‌به‌مو یکی است** (با diff ماشینی مقایسه شد؛ در `main` چون `dist/`
  نبود، یک بار با `dist/` روی همان درخت بازآزمایی شد). پس این کار **هیچ خطای تازه‌ای
  اضافه نکرد** و آن ۱۵ مورد، ازجمله بودجهٔ باندلِ ۱۵۹۲ کیلوبایتی، از قبل قرمز بودند.
  باندل خودِ برنامه تغییر نکرد (`src/` دست نخورد).

# ۲۰۲۶-۰۹-۲۴ (۴) — انتخابِ شبکه در والت پاپ‌آپ شد؛ آیکونِ تلفن از پلِ سولانا رفت؛ هشدارِ مدرنِ پروکسی بعد از انتخابِ فارسی؛ آهنگِ اختصاصیِ نوتیفیکیشنِ اندروید

جزئیات: [2026-09-24-network-popup-proxy-advice-notification-sound.md](2026-09-24-network-popup-proxy-advice-notification-sound.md).

- **انتخابِ شبکه در والت پاپ‌آپ شد.** لیستِ شانزده‌تایی از یک popover اینلاین (که کارتِ hero با
  `overflow: hidden` می‌بریدش و `transform`ِ `PageTransition` جایش را به‌هم می‌زد) به یک پنجرهٔ پورتال‌شده
  روی `document.body` رفت: پس‌زمینهٔ تار، کارتی که روی گوشی از پایین می‌آید و روی صفحهٔ عریض وسط
  می‌نشیند، هدر با دکمهٔ بستن، و قفلِ اسکرولِ صفحه. تپ روی پس‌زمینه می‌بندد و انتخاب نمی‌کند؛ تپِ داخل
  پنجره دیگر «بیرون» حساب نمی‌شود (`popRef` — پاپ‌آپ فرزندِ `rootRef` نیست).
- **آیکونِ تلفن از دکمهٔ «اتصال کیف پول سولانا» رفت.** آن آیکون می‌گفت «ادامه روی گوشیِ شماست»، در حالی که
  این دکمه صفحه را ترک نمی‌کند و شیتِ اتصال همان‌جا باز می‌شود — وعده‌ای که عملی نمی‌شد و «دکمه خراب است»
  خوانده می‌شد. برچسبِ متنی، که همان دستورِ درست است، تنها ماند.
- **هشدارِ مدرنِ پروکسی بعد از انتخابِ فارسی** (`ProxyAdvicePopup`، پورتال، `role="alertdialog"`): چون
  بیشترِ امکانات روی سرورهای خارج از کشور است، بهتر است با فیلترشکن (پروکسی یا VPN) وارد شد؛ و اگر نه،
  کندی از شبکه است نه از برنامه و دارایی در خطر نیست. فقط برای فارسی، و **گیت نیست** — ادامه در حالی که
  باز است کار می‌کند. ۴۰۰px عرض، ۱.۹ لیدینگ برای فارسی، ویژگی‌های منطقی برای RTL، کهربایی برای هشدار.
- **آهنگِ اختصاصیِ نوتیفیکیشن.** `res/raw/fbt_notification.wav` (۱.۳۵ ثانیه، سنتز‌شده با
  `scripts/gen-notification-sound.mjs`، بازتولیدپذیرِ بیت‌به‌بیت) روی دو کانالِ `MainActivity`
  (`fbt_alerts_v1` با `IMPORTANCE_HIGH` — همان که نوتیفیکیشن را از بالا می‌آورد — و `fbt_updates_v1`)،
  با `default_notification_channel_id` در مانیفست، `sound`/`channelId` در `server/fcm.js` (که در هر دو
  شاخه `'default'` می‌فرستاد)، و همان در `lib/notify.js` و `capacitor.config.json`.
