# ۲۰۲۶-۰۹-۲۴ (۳) — بنرِ فلیپِ سهام: تب‌ها حذف، ایکونِ تعویض داخل بنر، هر ۴۰ ثانیه؛ فارسی فقط برای فارسی؛ «چطور کار می‌کند» فروشگاه به زبان کاربر

جزئیات: [2026-09-24-stocks-flip-banner-and-shop-language.md](2026-09-24-stocks-flip-banner-and-shop-language.md).

- **تب‌های بنر فلیپ حذف شدند.** دو تبِ `RWA`/`افق جهانی` که بالای کارت بودند رفتند و جایشان یک دکمهٔ فقط-آیکون، بدون عنوان، داخل بنر و در گوشهٔ بالا-چپ آمد (`inset-inline-end` — در RTL یعنی چپِ فیزیکی). حلقهٔ شمارش معکوس دور همان آیکون جای نوار پیشرفت را گرفت.
- **چرخش خودکار هر ۴۰ ثانیه** (`AUTO_MS`: ۱۲ → ۴۰). باگی هم بسته شد که روی گوشی اولین تپ، hover مصنوعی می‌ساخت و چرخش خودکار را برای همیشه خاموش می‌کرد.
- **فارسی فقط برای فارسی.** `isEn` دیگر از RTL بودنِ چیدمان حدس نمی‌زند؛ با عربی و اردو کارت متن فارسی نشان می‌داد. حالا فقط `fa` فارسی است و بقیه انگلیسی.
- **«چطور استفاده کنم» فروشگاه به زبان کاربر.** `server/shop.js` همیشه `lang=en` می‌فرستاد. حالا به زبان کاربر می‌پرسد، و اگر Cryptorefills آن زبان را نداشت (مثل `fa` که `rich_description: null` می‌دهد) به انگلیسی برمی‌گردد و زبانِ مُرده را برای همان نمونه به‌خاطر می‌سپارد — هشدارِ قفلِ منطقه‌ای هرگز گم نمی‌شود. `contentLocale` می‌گوید واقعاً چه زبانی برگشت.
- **گام‌های فارسیِ خودمان** (۵ گام: مبلغ، تحویل به کریپتوریفیلز، پرداخت، کد با ایمیل، محل استفاده) وقتی متنِ ارائه‌دهنده در زبان کاربر نباشد، و متن اصلی با برچسب زبانش زیرش می‌ماند.

# ۲۰۲۶-۰۹-۲۴ (۲) — وام سولانا: موقعیت‌های کیف پولِ تازه، خواندنِ تازه بعد از تراکنش، باکسِ مقدارِ درست؛ «اسکرین گرفتید؟» به‌جای نوار برندِ همیشگی

جزئیات: [2026-09-24-solana-loan-positions-and-screenshot-prompt.md](2026-09-24-solana-loan-positions-and-screenshot-prompt.md).

- **وام سولانا:** `getUserVanillaObligation` برای کیف پولِ بدون سپرده throw می‌کند و به‌عنوان خطای RPC خوانده می‌شد ← `readVanillaObligation` آن را «موقعیتی ندارد» می‌خواند. بعد از هر تراکنش موقعیت با `refresh=1` از کشِ سرور رد می‌شود. snapshotِ مرورگری که کیف پول را نخوانده منتظر درِ سرور می‌ماند.
- **باکس مقدار:** دیگر با دکمه در یک ردیف نیست (`.btn { width: 100% }` آن را تا یک قرص کوچک فشرده می‌کرد) — تمام‌عرض، ۵۶px، با نماد دارایی؛ اعداد فارسی پذیرفته می‌شوند.
- **نوار برند حذف شد.** بعد از اسکرین‌شات (اندروید: `ScreenCaptureCallback` / MediaStore؛ دسکتاپ: PrintScreen و میان‌برها) بنری ۳۰ ثانیه‌ای پیشنهاد می‌دهد تصویر را با واترمارک آدرس سایت به اشتراک بگذارید.

# ۲۰۲۶-۰۹-۲۳ (۳) — تبِ سواپِ سولانا: «موجودی کم یا RPC را چک کنید» دروازه‌ای بود که یک خوانده‌ی تک‌شانسی می‌بست

گزارش کاربر، با دیاگنوستیکِ خودش: «اتصال به کیف پول درست است هم در اصلی هم سولانا، اما برای امضا و
خرید با سولانا در بیشتر اوقات خطای "موجودی کیف پول کم یا RPC را چک کنید" نمایش داده می‌شود.» و همان
دیاگنوستیک نصفِ دوم را رد می‌کند: `transport=standard`، `MWA PASS registered`،
`signAndSendTransaction` موجود، آدرس `AMU6pRs8Hs9FEcA2hgcvrWeVprqvMPSJoQzEkB3kqFoR`.
جزئیات کامل با جدولِ علت‌ها در
[2026-09-23-solana-swap-balance-read-fallback.md](2026-09-23-solana-swap-balance-read-fallback.md).

- **ریشه:** اتصال خراب نبود؛ **خواندنی** خراب بود که *قبل از امضا* دروازه‌بانی می‌کرد. یک lookup
  موجودی از گوشی به **یک** نود عمومی، و حکمِ دکمه از جوابِ همان یک خوانده. پنج نقص روی آن مسیر:
  دسیمالِ توکنِ واردشده **حدس** زده می‌شد (`decimals ?? 9` → برای مینتِ ۶-دسیمالی ۱۰۰۰ برابر
  کوچک‌تر)؛ مینت‌های **Token-2022** با فیلترِ `spl-token` خوانده می‌شدند (موجودیِ واقعی = دقیقاً ۰)؛
  این خوانده از لایه‌ی سخت‌شده‌ی RPC **رد** می‌شد (بدون timeout/failover/خطای نام‌دار، و
  `SOLANA_RPC_URL` نادیده)؛ موجودیِ خوانده‌نشده به‌عنوان **خرابیِ کیف پول** گزارش و دکمه غیرفعال
  می‌شد؛ و چکِ گس rentِ ساختِ اکانتِ مقصد (۲٬۱۰۰٬۰۰۰ لامپورت) را حساب نمی‌کرد.
- `src/lib/solana/chainReads.js` (تازه) — لایه‌ی خواندنِ خالص با transportِ تزریق‌شده: SOL با
  **رشته‌ی دقیق** (لامپورت `BigInt` تا انتها، چون `Number` بالای ۲^۵۳ بی‌صدا دقت از دست می‌دهد)،
  `spl-token` و `token-2022` **با هم** خوانده می‌شوند، صفر فقط بعد از تأییدِ مینت باور می‌شود،
  دلیلِ هر نود نگه داشته و در یک کدِ نام‌دار رتبه‌بندی می‌شود
  (`BLOCKED > RATE_LIMITED > TIMEOUT > ERROR > UNAVAILABLE`)، و هر خوانده `signal` می‌گیرد تا
  بازنده‌ی مسابقه دیگر به نودها ضربه نزند. سه متدی که روی سیم می‌روند —
  `getAccountInfo`، `getBalance`، `getParsedTokenAccountsByOwner` — هر سه در allowlistِ رله‌ی
  فقط-خواندنیِ همان روز هستند، پس دو لایه با هم جور می‌مانند.
- `server/solanaChainReads.js` + `GET /api/solana/{balances,token-info}` (تازه) — **درِ دوم** برای
  همان خوانده، از IP سرور: یک درخواست برای کلِ خوانده (به‌جای سه hop)، لامپورت/مقدار به‌صورت
  **رشته**، dedupe‌ی ۱٫۵ ثانیه‌ای، کشِ کوتاه برای شکست و بلند برای موفقیت، و ۵۰۲ صادقانه‌ای که
  جدولِ per-host را با خودش می‌آورد. مثلِ رله، **هرگز broadcast نمی‌کند** و پروکسیِ باز نیست
  (پارامترِ upstream ندارد؛ `SOLANA_RPC_URL` یا همان فهرستِ عمومی).
- `src/lib/solana/balanceSource.js` (تازه) — دو در، **اولین جوابِ صادقانه می‌برد**: دستگاه از مسیرِ
  `solanaRpcCall` (با همان cooldown/راهنمای «عمومی‌ها بسته‌اند» که امروز به `main` آمد)، و ~۱٫۲
  ثانیه بعد بک‌اندِ خودمان. درخواستِ بازنده abort می‌شود، در شکستِ کامل نودِ برنده‌ی کش‌شده ریست
  می‌شود، و نتیجه همیشه `via`/`serverTried`/`serverCode`/`hosts` را دارد. **هیچ‌وقت exception
  نمی‌اندازد**: `ok:false` + `code` قرارداد است.
- `src/lib/solana/swapPreflight.js` (تازه) — موجودیِ خوانده‌نشده **اعلان** است نه دکمه‌ی مرده
  (تصمیم با شبیه‌سازیِ خودِ کیف پول/چین)؛ **مقایسه‌ی مقدار فقط وقتی scale تأییدشده باشد**؛ گس =
  `۲۰٬۰۰۰` + (وقتی اکانتِ مقصد نیست) `۲٬۱۰۰٬۰۰۰` + مقدارِ ورودی وقتی ورودی SOL است، با
  `need/have/shortfall` دقیق تا UI بتواند کسریِ واقعی را بگوید.
- `src/pages/SolanaSwap.jsx` — دسیمال **حل** می‌شود (بک‌اند → مینتِ روی چین → فهرستِ Jupiter)،
  هرگز `?? 9`؛ تا حل‌نشدن، دروازه‌ی مقدار ساکت است و ردیف «تأییدنشده» نشان داده می‌شود؛ خواندنِ
  تکراریِ بی‌تغییر skip می‌شود؛ `balanceCode`/`balanceHosts`/`preflightNotice` به UI می‌رسند و
  `solanaWallet.js` هم `getSolanaTokenInfo` گرفت و `getSolanaBalance` دقیق شد.
- `src/lib/solana/health.js` + `src/components/WalletHealthPanel.jsx` — ردیفِ تازه‌ی **Chain read**
  بالای ردیفِ امضا و فیلدِ `chainRead` در JSONِ کپی‌شده: کدام در جواب داد (`rpc` یا `server`)، کدام
  host، چند ms، چند فراخوانی، چند SOL — و در شکست، کدِ نام‌دار + وضعیتِ بک‌اند + دلیلِ هر نود.
  گزارشِ بعدی مسیرِ خراب را **نام** می‌برد به‌جای حدس زدنش.
- `public/sw.js` — شل به `fbt-shell-v25`: هر چهار تغییر بالا سمتِ کلاینت هستند و نصبِ pinned به
  v24 بدون این rename بایت‌به‌بایت همان کدِ قدیم را اجرا می‌کند.
- تست‌ها — `test/solana-swap-chain-reads.test.js` (تازه، ۳۶ چک: لایه‌ی خواندن، preflight، هر دو در،
  لغوِ بازنده، تأییدِ صفرِ Token-2022، skipِ خواندنِ تکراری) با `npm run test:solana-swap`؛
  `test/wallet-diagnostics-probe.mjs` ۱۵۴/۱۵۴؛ یازده سوئیتِ سولانا/وام/پنل **۲۲۱/۲۲۱** (شاملِ
  `loan-solana-relay` و `loan-solana-relay-client` که همان روز به `main` آمدند)؛ `vite build` سبز.
  ممیزیِ `test/wiring.mjs` روی همین ماشین و همان فلگ‌ها: `origin/main` → ۲۶۹۹ ادعا/۱۴ خطا،
  این برنچ → **۲۷۰۹ ادعا (۱۱ تایِ تازه برای این فیکس)/همان ۱۴ خطا، با همان نام‌ها**؛ باندلِ
  first-paint ۱۵۵۹→۱۵۸۰ کیلوبایت (+۲۱). رatchetِ `<۱۳۶۰` روی `main` هم قرمز بود — driftِ قدیمی.
- **عمداً دست نخورد:** regexِ خطاهای `signAndSendSolana` هنوز بعضی شکست‌های broadcast را به
  `INSUFFICIENT_BALANCE` نگاشت می‌کند (علتِ موردِ ۵ حالا از قبل چک می‌شود، ولی خودِ نگاشت با مدرکِ
  سطحِ broadcast یک پچِ جدا می‌خواهد)؛ و `/.well-known/assetlinks.json` همچنان ۴۰۴ است چون به
  SHA-256ِ **کیستورِ ریلیز** نیاز دارد — `ci/assetlinks-publish-step.yml` آماده است و فقط باید در
  `.github/workflows/build-apk.yml` paste شود (توکنِ اتوماسیون اجازه‌ی نوشتن در workflow ندارد).

# ۲۰۲۶-۰۹-۲۳ (۲) — چرا رلهٔ خودِ برنامه «پاسخی که نتوانستیم استفاده کنیم» می‌داد: نامِ متد، از سمتِ اشتباهِ API

گزارش دوم همان روز از تبِ سولانای صفحهٔ وام: «یک یا چند گره سولانا درخواست را رد کردند
(۴۰۳ …)» · «RPC سولانا یا بازار Kamino خوانده نشد» · و در فهرستِ گره‌ها
**«رلهٔ خود برنامه — آن گره پاسخ داد، ولی پاسخی که نتوانستیم استفاده کنیم.»** آن خطِ آخر
خودش اعترافِ باگ بود: رله جواب داده بود. جزئیات کامل در
[docs/LOAN-RELAY-WIRE-METHODS-2026-09-23-FA.md](docs/LOAN-RELAY-WIRE-METHODS-2026-09-23-FA.md).

- **ریشه:** allowlist رله از **نام‌های JS** نوشته شده بود. `KaminoMarket.load()` اول با
  `getAccountInfo` بازار را می‌خواند (رله این را سرو می‌کرد) و بعد `loadReserves()` با
  `connection.getMultipleAccountsInfo(...)` ذخایر را — و `@solana/web3.js@1.98.4`
  (`index.cjs.js:6392/6410`) روی سیم **`getMultipleAccounts`** می‌فرستد. allowlist
  `getMultipleAccountsInfo` داشت و `getMultipleAccounts` را **نه**؛ پس دومین فراخوانیِ
  هر بارگذاری بازار با `-32601 … is not relayed` برمی‌گشت، بازار نیمه می‌ماند و صفحه —
  به‌درستی — هیچ تراکنشی نمی‌ساخت. جملهٔ «گره پاسخ داد ولی استفاده نشد» هم دقیقاً از همین
  می‌آمد: `-32601` در هیچ‌کدام از واژه‌های موجودِ `classifyNodeFailure` نمی‌گنجید و به
  `RPC_UNAVAILABLE` می‌افتاد، یعنی کاربری که مسیرش همهٔ نودهای عمومی را هم رد کرده بود،
  توصیهٔ «RPC خودت را در تنظیمات وارد کن» می‌گرفت برای باگی که در همین ریپو بود.
- `server/solanaRpcRelay.js` — `getMultipleAccounts` به allowlist اضافه شد (weight ۳،
  کشِ ۲.۵ ثانیه مثل بقیهٔ خواندن‌های بازار)، `RELAY_METHOD_ALIASES` + `resolveRelayMethod()`
  نام‌های سطحِ JS (`getMultipleAccountsInfo`, `getParsedTokenAccountsByOwner`) را به
  متدِ واقعی نگاشت می‌کنند **تا سرو شوند، نه رد** (یک متد، یک کلیدِ کش، یک حافظهٔ رد،
  یک بودجه)، و ردِ allowlist حالا ماشین‌خوان هم هست:
  `error.data = { relay: true, stage: 'allowlist', method }`.
- `src/lib/solanaRpc.js` — کلاسِ `UNUSABLE` در نقشهٔ cooldown (۱۰ دقیقه): پاسخی که ۲۰۰ است
  ولی JSON-RPC نیست دیگر **warm** نمی‌ماند و هر refresh اول از او پرسیده نمی‌شود.
  `solanaPublicsBlocked()` «هیچ‌کدام از عمومی‌ها نمی‌توانند» را از ترکیبِ رد **و** بی‌استفاده
  می‌فهمد و همان را در راهنمای persisted هم می‌نویسد — چون گزارش امروز مخلوط بود
  (سه ۴۰۳، یک ۴۲۹، یک خطای اتصال، دو پاسخِ بی‌استفاده) و قاعدهٔ قبلی («همه BLOCKED»)
  از آن هیچ نمی‌آموخت. ۴۲۹ همچنان حکمِ مسیر نیست و خطای شبکه‌ای هم نه.
- `src/lib/solanaLending.js` + ۱۲ زبان — `RELAY_METHOD_REFUSAL_RE`: ردیفی که **خودِ رله**
  متد را رد کرده (`-32601` یا متنِ «is not relayed») کدِ `RELAY_METHOD_UNAVAILABLE` می‌گیرد
  اگر و فقط اگر آن ردیف **خودِ رله** باشد (`isRelayUrl` روی URL آن تلاش)؛ همان متن از یک
  نودِ عمومی این برچسب را نمی‌گیرد، و ردِ رله سرصفحه (کدِ خطا) را هم برمی‌دارد فقط وقتی
  **همهٔ** تلاش‌ها ردِ رله بوده باشند. جملهٔ جدید در همهٔ زبان‌ها اضافه شد: «رلهٔ خودِ برنامه این درخواست
  را عبور نمی‌دهد — این نقصِ همین نسخهٔ اپ است، نه خرابی گره یا شبکه. چیزی ارسال نشد؛
  اپ را به‌روزرسانی کن.» `noteRefusedCandidates()` هم پاسخ‌های بی‌استفاده را به لایهٔ RPC
  یاد می‌دهد (به‌عنوان `UNUSABLE`). رله همچنان **فقط-خواندنی** است: `sendTransaction` در
  allowlist نیست و `getSolanaRpcUrl()` هرگز رله را برنمی‌گرداند.
- تست‌ها — `test/loan-solana-relay.test.js`: فهرستِ «خواندن‌های موردنیاز وام» از نام‌های JS
  به **نام‌های روی سیم** تغییر کرد (همان فهرستی که باگ را پنهان کرده بود) + سه تستِ تازه،
  از جمله یک `Connection` **واقعیِ** web3.js با fetch تزریقی که `getMultipleAccountsInfo`
  را صدا می‌زند و باید سرو شود و متدی که به بالادست می‌رسد باید `getMultipleAccounts` باشد؛
  `test/loan-solana-relay-client.test.js`: ردیفِ رله → `RELAY_METHOD_UNAVAILABLE`، همان متن
  از نودِ عمومی → هیچ‌کدام، ۲۰۰ غیرِ JSON-RPC → `UNUSABLE` و فهرستِ کاملِ آن‌ها → رله
  جلوی صف؛ `test/loan-errors-l10n.test.js`: کدِ جدید در فهرستِ کدهای سطحِ وام.
- `public/sw.js` → `fbt-shell-v24` (بدون این، دستگاهی که اپ را باز کرده همان کدِ قدیم را
  اجرا می‌کند و این خطا بایت‌به‌بایت تکرار می‌شود).
...
از FBT می‌برد و اتصال بعد از یک «ادامه» و تأیید دوم کامل می‌شد؛ در APK کاربر بعد
از تأیید به اپ برمی‌گشت اما پنجره برای همیشه روی انتظار می‌ماند.

- **وب:** `wcMetadata()` دیگر برای صفحهٔ وب `redirect.universal` اعلام نمی‌کند.
  آن مقدار آدرس هویت dApp نبود؛ دستور بازکردن دوبارهٔ سایت داخل مرورگر والت بود.
  تب اصلی و SignClient آن حالا زنده می‌ماند و همان پاسخِ اولین تأیید را می‌گیرد.
- **APK:** پل بومی ابتدا URI اختصاصی والت (`trust://wc?uri=…`) را با package دقیق
  باز می‌کند و `wc:` خام فقط fallback است؛ بنابراین Trust وارد صفحهٔ واسط
  «برای اتصال ادامه دهید» نمی‌شود.
- **چرخهٔ حیات اندروید:** `MainActivity.onResume()` سیگنال واقعی بازگشت اپ را به
  WebView می‌دهد. نشستِ درحال اتصال همان Core Relayer را با `transportOpen()`
  بیدار و subscriptionها را بازیابی می‌کند تا پاسخ تأییدی که هنگام freeze شدن
  WebView روی relay مانده بود به همان promise برسد. pairing تازه ساخته نمی‌شود؛
  پس این بازیابی نمی‌تواند تأیید دوم تولید کند.
- **تست:** قرارداد redirect وب/APK، ترتیب branded-before-raw در Java، سیگنال
  lifecycle و بیدارشدن transport به probe WalletConnect افزوده شد.

# ۲۰۲۶-۰۹-۱۹ — هشدار «این dApp کلاهبرداری است» در والت: علت‌ها و اصلاح‌ها

گزارش: «وقتی می‌خواهم EVM یا سولانا را در والت‌هایی مثل تراست‌والت امضا کنم — بخصوص
سولانا مثل فانتوم — ارور می‌دهد: این dApp به نظر می‌رسد کلاهبرداری باشد و می‌گوید
خارج شو یا با مسئولیت خودت ادامه بده.»

**۱. هویت والت = همان دامنه‌ای که صفحه روی آن باز است (`walletIdentityUrl` در
`src/lib/wc/config.js`):** تا امروز `wcMetadata()` همیشه `https://fbtswap.ir` را
اعلام می‌کرد، حتی وقتی صفحه روی preview، سندباکس یا هر میزبان دیگری باز بود. ولی
WalletConnect هویت را متقاطع چک می‌کند: `Verify.register()` خودِ
`window.location.origin` را به سرور تأیید می‌فرستد و SDK از آن
`validation = attestedOrigin === new URL(metadata.url).origin ? 'VALID' : 'INVALID'`
را می‌سازد. یعنی اعلام دامنهٔ دیگر از روی صفحهٔ دیگر، «عدم تطابق دامنه» است —
همان صفحهٔ قرمز «این dApp مشکوک است» با دکمهٔ ادامه. حالا: صفحهٔ واقعی روی https
خودش را اعلام می‌کند (پروداکشن: `https://fbtswap.ir`، روی preview: دامنهٔ خودِ
preview) و فقط جایی که والت نمی‌تواند برود — اپ بستهٔ اندروید (`https://localhost`)،
لوکال‌هاست دولوپمنت، صفحهٔ `http://` — هویت عمومی اعلام می‌شود. `wcMetadata(view)`
و `walletIdentityUrl(view)` پنجره را به‌عنوان آرگومان می‌گیرند تا بدون مرورگر
تست‌شدنی باشند.

**۲. سولانا: والت خودش ارسال می‌کند، نه ما (`signSendConfirm` در
`src/lib/launch/solana/signing.js`):** این تابع ترجیح می‌داد `provider.signTransaction`
بگیرد و بعد خودش با `connection.sendRawTransaction` بفرستد — همان الگویی که
موتورهای امنیتی (Blowfish که فانتوم، بک‌پک، سولفلر و تراست از آن می‌خوانند)
به‌عنوان رفتار drainer علامت می‌زنند؛ توصیهٔ رسمی خودشان برای برداشتن این هشدار
استفاده از `signAndSendTransaction` است. اکنون روی هر دو مسیر (provider تزریقی و
MWA با فیچر `solana:signAndSendTransaction`) خودِ والت می‌فرستد و امضا-سپس-ارسال
فقط مسیر جایگزین است، برای والتی که یک تراکنش نیمه‌امضاشدهٔ create را نمی‌پذیرد.
رد کاربر هرگز با پرامپت دوم دوباره پرسیده نمی‌شود.

**۳. گزارش سلامت، هویت والت را نشان می‌دهد (`src/lib/wc/health.js` +
`WalletHealthPanel`):** ردیف «هویت کیف پول (چیزی که به والت گفته می‌شود)» که
`declared` و `pageOrigin` را کنار هم می‌گذارد؛ وقتی این دو یکی نباشند، هر دو را
چاپ می‌کند به‌جای یک برچسب مبهم. کلیدهای `wallet.healthIdentity*` به en/fa اضافه شد.

**۴. دو ابزار اندازه‌گیری:** `scripts/walletconnect-domain-verify.mjs` کد تأیید
مالکیت دامنه را در `public/.well-known/walletconnect.txt` می‌نویسد (وضعیت فعلی:
نبود این فایل یعنی همهٔ والت‌ها دامنه را «UNVERIFIED» می‌بینند) و
`scripts/wallet-reputation-check.mjs` دامنه را روی فهرست‌های عمومی (MetaMask
eth-phishing-detect، ScamSniffer، ChainPatrol، اختیاری Google Safe Browsing) و روی
همان دو URLی که خودِ والت می‌گیرد (آیکون و فایل تأیید) چک می‌کند. هر فهرستی که
جواب ندهد `UNKNOWN` گزارش می‌شود، هیچ‌وقت PASS.

**بررسی زنده (همین امروز، از خود مخازن):** `fbtswap.ir` در `phantom/blocklist`
(۲٬۳۱۹ ورودی)، در `MetaMask/eth-phishing-detect` (۲٫۸MB) و در ScamSniffer (۹MB)
**نیست**، و fuzzylist فانتوم خالی است. پس هشدار از «لیست سیاه» نبود؛ از عدم تطابق
هویت (بند ۱) و/یا امتیاز دامنهٔ سرویس‌های غیرعمومی (Blowfish/Blockaid) می‌آمد که
مسیر رفعشان فقط درخواست رسمی است — کل مسیر با متن ایمیل آماده در
`docs/WALLET-SCAM-WARNING-FA.md`.

**تست:** `test/walletconnect-stack-probe.mjs` شانزده بررسی جدید (کل ۳۲۵): هویت روی
پروداکشن، ساب‌دامین، preview، سندباکس، اپ بسته، لوکال‌هاست و `http`، یکسانی هویت و
آیکون و redirect، و اینکه هیچ ورودی‌ای هویت `localhost` نمی‌دهد.

# ۲۰۲۶-۰۹-۱۹ — پل بومی: راهنمای مدرن + امضای واریز در همین صفحه (منبع‌های EVM)

گزارش‌ها: «باکس راهنمای پل بومی را مدرن و زیب کن و فاصلهٔ باکس را از باکس
بالایی و پایینی درست کن» · «نمیشه در خود سایت ما انجام شود به جای گذاشتن
memo و ادرس در تورچین در خود سایت باشد و مشتری امضا کند فقط».

**۱. باکس راهنمای پل بومی (`ThorDepositGuide` + `bridge-safety.css`):**
پنج مرحله از فهرستِ خشک به تایم‌لاین مدرن تبدیل شد — نشانِ گرادیانی با
آیکون برای هر مرحله، ریلِ اتصالِ سایان، شمارهٔ مونو، هشدار به‌صورت نوارِ
قرمزِ آیکون‌دار (نه پاراگراف پنجم)، دکمه‌سازیِ دو خروجی (THORSwap /
مستندات) و پوستهٔ اختصاصی با گلوی سایان برای خودِ جعبه. فاصله‌ها درست شد:
باکس به باکس بالایی «جوش‌خورده» بود چون `.thor-guide` یک div ساده بود و
قانون `:first-child` مارجینِ InfoBox داخلی را صفر می‌کرد و قانونِ
`.infobox + .infobox` هم به‌خاطر همان wrapper هرگز فعال نمی‌شد — حالا
`.thor-guide { margin: 12px 0 }` مالک فاصله است و کل ریتم عمودی تب ۱۲px
شد. اضافه‌بر آن: خطای POOLS_FAILED دیگر کل تب را به یک جملهٔ قرمز
نمی‌کاهد — راهنمای استاتیک می‌ماند. تم روشن هم پشتیبانی شد.

**۲. امضای واریز THORChain در همین صفحه (`src/lib/thorEvmDeposit.js`
جدید + `ThorPanel` + روت سرور `/api/thor/tx/:hash`):** جواب عمیق به «مشتری
فقط امضا کند»: برای منبع‌های EVM (ETH/BSC/AVAX/Base) واریز THORChain یک
فراخوانی قرارداد معمولی است که کیف پول متصلِ همین صفحه می‌تواند امضا
کند — پس `depositWithExpiry(vault, asset, amount, memo, expiry)` با
خزانه/memo/انقضای همانِ قیمت ساخته می‌شود، کاربر یک شیت بازبینی می‌بیند و
کیف پول خودش امضا می‌کند. توکن‌ها: مجوزِ دقیقِ همان مبلغ به روتر (هرگز
بی‌نهایت؛ ریست-به-صفر قبل از تغییر مجوز). همهٔ گاردهای مسیر پول:
`validateBridgeRequest` + `assertBridgeContracts` (روتر باید کد داشته باشد)
+ `assertBridgeSigner` قبل از هر امضا + تطبیق گیرندهٔ memo با آدرس روی
صفحه + مارجین ۱۲۰ ثانیهٔ انقضا + هضمِ آدرس‌های `0X` بزرگ THORChain به
lowercase. کارمزد وابسته (affiliate) روی منابع EVM در همان memo می‌ماند.
برای خانوادهٔ بیت‌کوین/کازموس امضا در سایت ممکن نیست (UTXO/OP_RETURN و
پیام کازموس را کیف پول EVM نمی‌سازد) و مسیر دستی، صادقانه، سر جای خودش
ماند. بعد از ارسال: ردیاب زندهٔ چهارسطحی (دیده‌شد → نهایی‌شد → سواپ شد →
پرداخت شد) از `/api/thor/tx/:hash` که همان failover چند‌نودی سرور را دارد؛
۴۰۴ِ «هنوز دیده نشده» = «در انتظار»، نه خطا. امتیاز bridge با شاهد txHash.

**تست:** `test/thor-evm-deposit.test.js` (۱۱ vitest: نگاشت زنجیره، تبدیل
BigInt دقیق، تطبیق memo، نرمال‌سازی وضعیت، مسیر پول با ethers ساختگی —
value بومی، مجوز دقیق، یک بازبینی، هر ردُس) + ۴ تست UI جدید در
`bridge-native-ui.test.jsx` (کارت امضا برای منبع EVM، رد کردن امضا، درخواست
اتصال کیف پول، عدم پیشنهاد امضا برای BTC). `test:bridge-safety` کامل: ۵۵/۵۵
سبز؛ بیلد پروداکشن سبز. ترجمهٔ کامل en/fa/ar برای `thor.sign.*` و متن‌های
به‌روز‌شدهٔ what2/manual (دیگر «فقط قیمت» نیست).

# ۲۰۲۶-۰۹-۱۸ (۴) — سه اصلاح: alert قیمتیِ صادق، خطاهای پل به ۱۲ زبان، پایانِ «نصفش نیست»

گزارش‌ها: «بیت‌کوین ۴۰ درصد رشد کرد که در واقعیت ۴ درم رشد کرده» · «گاهی
استرینگ هست» / «نبود موجودی وقتی کیف مول وصله» · «تا مرورگر کیف مول‌ها
مهتابی بیشتر و گاهی صفحه تا لود بشه نصفش نیست انگار باگ داره». جزئیات کامل
و شواهد: `docs/FIXES-ROUND-2-FA.md`.

**۱. alert قیمت (`src/lib/priceAlerts.js`):** منطق درست بود، ورودی‌ها نبود.
baseline از snapshot آفلاین (بذر BTC: ۶۷,۴۵۰) با قیمت زنده (یا برعکس) مقایسه
می‌شد. سه گارد: سطرِ آفلاین هرگز baseline/alert نمی‌شود؛ تغییر منبع یا
baseline قدیمی (بدون `src`، شکل legacy) → ثبتِ بی‌صدا از نو؛ قدیمی‌تر از ۷
روز → از نو؛ حرکت >۳۵٪ در پنجرهٔ <۶ ساعت → خطای داده، از نو مسلح می‌شود.
حرکت واقعی، cooldown و رفتار fixture ها دست‌نخورده. ثابت‌ها
(`STALE_BASELINE_MS`, `SHORT_WINDOW_MAX_PCT`, `SHORT_WINDOW_MS`) صادر شدند؛
کلید store همان `fbt-price-alert-base-v1` ماند.

**۲. خطاهای پل (`src/lib/bridgeErrors.js` + ۱۲ locale):** همهٔ تب‌ها با
`defaultValue: code` ترجمه می‌کردند و فقط ۵ کلید وجود داشت؛ کدهای ماشین
(`INSUFFICIENT_BALANCE`, `USER_REJECTED`, `PROVIDER_BAD_RESPONSE`, …) خام
چاپ می‌شدند و quote های ناموفق به «مسیری پیدا نشد» فرو می‌ریختند. حالا ۲۸
کد در en/fa/ar + ۹ زبانِ دیگر (مسیر `gen-locales`) ترجمه شده؛ کد ناشناخته
داخل جملهٔ عمومی برای پشتیبانی می‌ماند؛ نثرِ کیف پول/سرور سطرِ شاهدِ
`dir="ltr"` می‌شود نه تیتر؛ کدهای `crossChain.err.*` پویا resolve می‌شوند.
سیم‌کشی: `Bridge.jsx` (سه مسیر)، `TronPanel.jsx` (دو مسیر)،
`CrossChainDesk.jsx` (سه نقطه). ضمن ممیزی: `Swap.jsx` کلیدهای
`wallet.emailChain*` را با پیشوند `swap.` صدا می‌زد و کلید خام نشان می‌داد —
درست شد.

**۳. کندی (`src/index.css`, `src/App.jsx`):** کی‌فریم‌های `drift-*` روی
orb های `blur(70px)` هر فریم `scale()` داشتند = بازرسمِ بافت روی main
thread؛ scale حذف شد و حرکتِ خالصِ translate3d روی compositor ماند. fallback
اسپینر (۵۵vh خالی) با اسکلتِ صفحه‌شکل `.skel-page` عوض شد — همان ارتفاع را
نگه می‌دارد تا نوار پایین نپرد.

**تست:** probe جدید `test/bridge-notify-validation-probe.mjs` (۲۱ چک) داخل
`npm test` سیم‌کشی شد + `test/price-alerts-validation.test.mjs` (۹ vitest) +
۴ pin جدید در `wiring.mjs`. ران کامل روی بیس و روی شاخه: مجموعه‌ی خطاهای
قرمز یکسان (همه از قبل)، صفر رگرسیون؛ بیلد پروداکشن سبز (۱۴۱۲KB در برابر
۱۴۰۹KB بیس — بودجه از قبل رد شده بود).

# ۲۰۲۶-۰۹-۱۸ (۳) — حذفِ «ایمیل و ورود با سوشال»: یک نمونهٔ AppKit، یک مودال، سه مسیرِ اتصال

> درخواستِ مالک:
> «در صفحه اتصال والت ورود با سوشال را حذف کن هم داره به برنامه اسیب میزنه هم
> باگ زیاد داره هم داره به ورود با والت کانتکت هم اسیب میزنه بعد حذف بیار رو
> برنچ اصلی لایو بشه»
>
> دامنهٔ کار پیش از برش تأیید شد: **کلِ ردیف** برود — کیف پولِ توکارِ Reown
> (ایمیل + سوشال)، نه فقط دکمه‌های گوگل/اپل/X.

**چرا این حذف یک «رفعِ باگ» است، نه فقط کم‌کردنِ یک گزینه:** همهٔ باگ‌های این
سطح از یک تصمیمِ ساختاری می‌آمدند — لاگینِ ایمیل/سوشال داخلِ مودالِ
والت‌کانکت جا نمی‌شد، پس **نمونهٔ دومِ `createAppKit()`** گرفت و دو نمونه روی
singletonهای کنترلرها (`OptionsController`، `ConnectionController`,
`ChainController`) و روی **یک** عنصرِ `<w3m-modal>` می‌نشستند:

- «لیستِ کیف‌پولِ والت‌کانکت باز می‌شود و هیچ‌چیز وصل نمی‌شود»: سطحِ
  والت‌کانکت AppKit را با **صفر آداپتور** بالا می‌آورد و
  `noAdapters=true`/`manualWCControl=true` را قفل می‌کند؛ `ModalController.open()`
  پیش از دیدِ درخواستی همین‌ها را می‌خواند، پس تپِ ایمیل به `AllWallets` می‌رفت —
  گریدی که ردیف‌هایش `wcUri` می‌خواهند و این سطح هیچ جفت‌سازی نمی‌سازد.
- «پاپ‌آپِ ایمیل آدرس و موجودیِ یک والتِ مرده را نشان می‌دهد»:
  `activeCaipAddress`ِ مشترک بعد از چرخهٔ اتصال/قطعِ والت‌کانکت پاک نمی‌شد.
- «کادرِ ایمیل هست ولی نمی‌شود تایپ کرد»: `hasAnyConnection('AUTH')` روی
  رکوردی که هیچ account نداشت.
- «اپرو یا کنسل، صفحهٔ امضا برمی‌گردد»: چهار driverِ همزمان هر کدام
  `personal_sign` واقعی می‌زدند و هر بار `ApproveTransaction` باز و
  `rejectRpcRequests()` قبلی را باطل می‌کرد.
- «اجازهٔ تغییر شبکه نمی‌داد»: فریمِ امن فقط ۲۴ شبکه دارد و **۸ زنجیرهٔ از ۱۶
  زنجیرهٔ ما** رویش نیست.

برای هر کدام یک اصلاح نوشته شده بود و بعد اصلاحِ خودِ اصلاح
(`assertEmailRouting`, `assertEmailNetwork`, `clearPhantomAuthConnection`,
`resetSharedConnectionState`, پروبِ دومودی امضا، keeper، fast-attach،
rollbackِ مارکر). حذف، **علت** را برداشت: یک نمونه، یک مودال، یک مالکِ singleton.

## آنچه برداشته شد

- **`src/lib/wc/embedded.js` (۲۰۱۰ خط) حذف شد** و با او چهار تعمیرِ
  کنترلریِ مخصوصِ ایمیل در `appkit.js`، پنج کرانِ `email*` و `SECURE_SITE_URL`
  در `config.js`.
- **`WalletContext.jsx`:** مدِ `'email'`، `connectEmailSocial`،
  `restoreEmailSocial`، `retryEmailAttach`، keeperِ پس‌زمینه، `emailModalActive`،
  `switchChainResult` و «درگاهِ اول ایمیل» در بوت. سه مد باقی است
  (`injected` / `wc` / `local`)، یک مسیرِ attach، یک restore. مسیرِ attach دیگر
  fallbackِ «signer دست‌ساز» ندارد: providerِ والت‌کانکت که signer ندهد، خراب
  است و صادقانه `wc_attach_failed` می‌شود.
- **شیتِ اتصال:** ردیفِ «ایمیل و ورود با سوشال» و دو نوتیسِ خطای ایمیلی با
  دکمهٔ «تلاش دوباره». ترتیبِ فعلی: WalletConnect (توصیه‌شده) → کیف‌های
  تزریقی/EIP-6963 → کیفِ درون‌برنامه‌ای. شیت فقط برای **یک** مودال کنار می‌رود.
- **پنلِ سلامت:** پروبِ فریمِ `secure.walletconnect.org`، سقفِ مصرفِ پروژه
  (که کادرِ ایمیل را غیرفعال می‌کرد)، خلاصهٔ features داشبورد
  (`summarizeProjectConfig`) و فیلترِ پلتفرمِ سوشال‌ها. سه پیوندِ باقی‌مانده:
  پروژه، دامنه‌های مجاز، سوکتِ رله + کانالِ تحویل و واقعیت‌های ذخیره‌سازی.
- **برچسب‌ها:** `wallet.mode.email` در صفحهٔ کیف پول و کارتِ امنیت، و نوتیسِ
  تغییرِ شبکهٔ ایمیل در Swap.
- **۱۲ فایلِ زبان:** ۲۱ کلید حذف شد (`wallet.emailSocial*`,
  `wallet.emailSigningDenied*`, `wallet.emailProviderPending*`,
  `wallet.retryAttach`, `wallet.healthSecureSite`, `wallet.healthUsage*`,
  `wallet.healthProjectSource*`, `wallet.emailChain*`, `toast.emailSwitch*`,
  `wallet.mode.email`) و دو جمله بازنویسی شد (`wallet.healthHint` و
  `wallet.healthRelayFreeRoutes` حالا سه پیوند و دو مسیرِ بدونِ رله را
  می‌گویند). `coverage.json` با `scripts/gen-locales.mjs` از نو ساخته شد.

## کاربرِ قدیمی چه می‌بیند

- کلیدهای سطحِ حذف‌شده در اولین بوت پاک می‌شوند — **پیش از** هر بهداشتِ دیگری،
  چون یکی از آن‌ها در CONSTRUCTOR خوانده می‌شود: `W3mFrameProvider` با دیدنِ
  `@appkit-wallet/EMAIL_LOGIN_USED_KEY` فریمِ امن را می‌سازد، چه این صفحه سطحی
  برایش داشته باشد چه نه. `storage.js` حالا آن‌ها را می‌شناسد
  (`listEmbeddedWalletKeys` / `purgeEmbeddedWalletKeys`):
  `fbt_email_social_connected`، `@appkit-wallet/*`، `@appkit/social_provider`،
  `@appkit/connected_social`، `@appkit/recent_emails`.
- `isConnectionKey()` برای `@appkit-wallet/*` برگشت: قبلاً محافظت می‌شد (پاک‌کردنش
  یعنی لاگوتِ یک کاربرِ واقعی)؛ حالا که سطحی برای ورود نمانده، بقایای آن است و
  پاک‌سازیِ عمومی هم برش می‌دارد. پنلِ سلامت تعدادشان را با
  `legacy-email-keys=N` گزارش می‌کند.
- **صادقانه:** کیف پولی که پشتِ لاگینِ ایمیل/سوشال **ساخته** شده، کیفِ توکارِ
  Reown است و این نسخه دیگر راهی برای بازشدنش ندارد. چیزی از بین نمی‌رود (کلید
  غیرامانی است و هرگز سمتِ ما نبود)، ولی آن کاربر باید با والت‌کانکت، کیفِ
  تزریقی یا کیفِ درون‌برنامه‌ایِ جدید وصل شود. اگر مسیرِ مهاجرت خواسته شود
  (استخراجِ کلید / یک صفحهٔ «بازیابیِ کیفِ ایمیلی») کارِ جداگانه‌ای است.

## تست

- `test/walletconnect-stack-probe.mjs`: بخشِ ۹ **خودِ حذف** را قفل می‌کند —
  فایل نیست، هیچ فایلی importش نمی‌کند، سطحِ عمومی هیچ نمادِ ایمیلی export
  نمی‌کند، **هیچ فایلی `createAppKit()` صدا نمی‌زند**، سطحِ والت‌کانکت هنوز
  `features: { email: false, socials: false }` را اعلام می‌کند، هیچ ردیف/مد/رشتهٔ
  ایمیلی در شیت، کانتکست، صفحهٔ کیف پول و لوکال‌ها نمانده، و پاک‌سازی دقیقاً
  همان چهار کلید را برمی‌دارد (نه کیفِ محلی، نه نشستِ `wc@2:`، نه کش را).
  بخشِ ۱۵ سیم‌کشیِ جدید را می‌سنجد (ترتیبِ پاک‌سازی در بوت، یک restore، یک
  attach). مجموع: **۲۲۵ ادعا، همه سبز**.
- `test/wallet-health-panel.test.jsx` از نو نوشته شد: سه پیوند با عددِ
  اندازه‌گیری‌شده، میزبانیِ رله که HTTPS پاسخ داد و سوکت نه، و **هیچ** جملهٔ
  ایمیل/سوشالی روی DOM.
- حذف شدند: `test/email-routing-probe.mjs` (ربودنِ گرید — نمونهٔ دومی نمانده که
  بخواهد ببندد) و `test/inapp-wallet-email-session.test.jsx` (بازگردانیِ نشستی
  که اپ دیگر برنمی‌گرداند) + سیمِ همان در `test/run.mjs`.
- `test/wallet-connect-sheet-probe.jsx`: نبودنِ ردیفِ ایمیل/سوشال هم مثلِ بودنِ
  بقیهٔ ردیف‌ها روی DOM سنجیده می‌شود.

## مستند

`WALLET-SOCIAL-EMAIL-REMOVAL-2026-09-18.md` (گزارشِ کاملِ حذف) · به‌روزرسانیِ
`docs/WALLET-CONNECT-STACK-FA.md` (نقشهٔ ماژول‌ها، جدولِ گزارش‌ها، تست‌ها و
بازنشستگیِ بخشِ ۸) · بایگانیِ `docs/EMAIL-SOCIAL-LOGIN-FA.md`.

## داشبوردِ Reown

پروژهٔ `5997d5aee8bb42f43ddec4b1a5f94eb1`: **Email & Social** را می‌شود خاموش
کرد — هیچ‌جای اپ دیگر آن را نمی‌خواند. **Allowed Domains**
(`https://fbtswap.ir` و `https://localhost`) و **App IDs** (`ir.fbtswap.app`)
بدونِ تغییر و همچنان برای والت‌کانکت لازم‌اند.

---

# ۲۰۲۶-۰۹-۱۸ (۲) — «ورود ایمیل انجام شد، اما کیف پول در این صفحه آماده نشد»

> گزارشِ واقعی (اندروید ۱۰ / Telegram / `https://fbtswap.ir`):
> ورود با ایمیل/گوگل تیکِ سبز می‌خورد ✅ — اما بعد از آن صفحه‌ی
> «کیف پول در این صفحه آماده نشد» می‌آید و دکمه‌ی «تلاش دوباره» هم
> دوباره صفحه‌ی «کیف پول را وصل کنید» را باز می‌کند.
> `trace: email_sign_probe_failed {m:'NO_ACCOUNTS'}` (هر بار) و در پایان
> `email_attach_failed_pending` — در حالی که
> `storage: sdkLoginMarker:true · connectionStatus:'connected' · storedConnectors:['AUTH']`
> و `shared: isConnected:true · authAccounts:2`. یعنی نشست هست، ولی attach نمی‌شود.

**علت:** پروبِ امضا (`probeSigning`) اولین RPC خود یعنی `eth_accounts` را
پرسه‌ی «حکم» می‌گرفت: پاسخِ **آرایه‌ی خالی** = `NO_ACCOUNTS` و پروب تمام.
اما فریمِ امن (secure.walletconnect.org) وقتی نشستش برای زنجیره‌ای که dapp
می‌پرسد در حالِ بازسازی است، **آرایه‌ی خالی** برمی‌گرداند — پاسخِ معتبرِ یک
نشستِ زنده، نه خطا. سؤالِ واقعی («آیا این provider برای آدرسِ همان لاگین
**امضا** می‌کند؟») هرگز پرسیده نمی‌شد؛ سه بار retry هم همان حالت را دوباره
پروب می‌کرد و در نهایت `EMAIL_PROVIDER_PENDING` می‌شد و تنها راهِ خروجش
دکمه‌ای بود که کلِ جریانِ اتصال را از نو اجرا می‌کرد (همان پاپ‌آپِ «وصل کردن
کیف پول»).

حالا (`src/lib/wc/embedded.js`، `src/context/WalletContext.jsx`):

- **۱. پروب، سؤالِ واقعی را می‌پرسد.** اگر `eth_accounts` خالی بود و آدرسِ
  لاگین معلوم باشد، یک `personal_sign` روی پیامِ ثابتِ پروب انجام می‌شود
  (همان مسیرِ واقعیِ امضای فریم) و امضا **تأیید** می‌شود: باید با
  `ethers.verifyMessage` دقیقاً به همان آدرس برسد. نتیجه‌ها:
  `{ok:true, via:'personal_sign'}` · `SIGNER_MISMATCH` (امضای معتبرِ کلیدِ
  دیگر) · `NO_ACCOUNTS` (چیزی برای امضا نبود) · یا پیامِ خودِ فریم. ethers
  به‌صورت lazy لود می‌شود و اگر تأیید ممکن نباشد، جواب **آماده‌نبودن** است —
  آرایه‌ی خالی هرگز «وصل» نمی‌شود.
- **۲. `embeddedAccountSnapshot()` (جدید، در سطحِ عمومیِ استک).** آدرس و
  provider را که اپ **همین حالا** دارد در یک تیک می‌خواند (instance →
  کانکتورِ AUTH → `activeCaipAddress` → رکوردِ AUTH) و هیچ مودالی باز نمی‌کند.
- **۳. `retryEmailAttach()` — «تلاش دوباره» فقط attach را تکرار می‌کند، نه
  لاگین را.** بدون مودال، بدون ورودِ دوباره، روی همان نشست.
- **۴. کِیپرِ پس‌زمینه.** اگر پروبِ دوباره هم زود باشد، تا ۱۲ تلاش روی
  backoff (۱.۲ تا ۱۲ ثانیه، ≈۶۰ ثانیه) در پس‌زمینه ادامه می‌دهد و لحظه‌ای که
  فریم آماده امضا شد، کیف پول را وصل می‌کند، خطا را پاک می‌کند و به کاربر
  خبر می‌دهد («اتصال در پس‌زمینه ادامه دارد — همین صفحه کافی است»). با
  disconnect، تغییرِ mode و بسته‌شدنِ صفحه متوقف می‌شود
  (`email_keeper_attached` / `email_keeper_exhausted` در trace).
- **۵. UI.** در صفحه‌ی pending یک خطِ صادق اضافه شد
  (`wallet.emailProviderPendingHint` در هر ۱۲ زبان) و دکمه‌ی تلاشِ دوباره
  به مسیرِ سریع وصل شد.

مسیرِ WalletConnect، رله، QR و داشبورد دست نخورده‌اند؛ `isProviderUsable`
معنای قبلی‌اش را دارد و هیچ سیاستِ storage تغییر نکرده است.

تست‌ها: `node test/walletconnect-stack-probe.mjs` → ۳۵۲/۳۵۲ (با امضاهای
واقعیِ ethers) · `npx vitest run test/wallet-health-panel.test.jsx` → ۴/۴ ·
`node test/email-routing-probe.mjs` → ۱۷/۱۷ · `node test/about-locales-probe.mjs`
→ PASSED · `npm run build` → سالم.

# ۲۰۲۶-۰۹-۱۸ (۱) — «دیکانکت که می‌زنم و با ایمیل می‌روم، فقط یک پاپ‌آپ باز می‌شود که موجودی نشان می‌دهد»

> گزارشِ واقعی (اندروید ۱۰ / Telegram / `https://fbtswap.ir`):
> WalletConnect وصل می‌شود ✅ — اما ورود با ایمیل کار نمی‌کند؛ بعد از
> «دیسکانکت»، لمسِ «ایمیل و ورود با سوشال» فقط یک پاپ‌آپ باز می‌کند که
> موجودیِ کیف پولی را نشان می‌دهد که کاربر همین حالا خارجش کرده بود.
> `trace: email_appkit_ready → email_restore_none (+30s) → email_wait_timeout (+75s)`
> و `storage: appkitConnectionKeys:1 · orphanKeys:true`.

سه لایه‌ی مستقل، هر سه در مسیرِ «بعد از دیسانکت»؛ هیچ‌کدام ربطی به رله یا
داشبورد نداشتند (هر دو در گزارش سبز بودند).

- **۱. پاک‌سازیِ storage از کلیدهای AppKit ۱.۸.۱۹ خبر نداشت — علتِ «فانتوم با موجودی».**
  AppKit 1.8.19 وضعیتِ اتصال را زیر کلیدهایی می‌نویسد که فهرستِ شش‌تاییِ قبلی
  (`APPKIT_CONNECTION_KEYS`) هرگز نامشان را نشنیده بود:
  `@appkit/connections` (فهرستِ اتصال‌های ذخیره‌شده)، `@appkit/connection_status`،
  `@appkit/connected_namespaces`، `@appkit/active_namespace`،
  `@appkit/social_provider` و همراهان. یک دیسکانکت که پاک‌سازی شد «موفق» گزارش
  می‌شد، اما این کلیدها سر جایشان می‌ماندند و SDK از دیدِ خودش هنوز یک کیفِ
  ایمیلِ وصل داشت. نتیجه: لمسِ بعدیِ «ورود با ایمیل» به‌جای فرمِ ایمیل، همان
  کیفِ «خارج‌شده» را با آدرس و **موجودی** و یک کادرِ ایمیلِ **غیرفعال** نشان
  می‌داد — چون خودِ `w3m-email-login-widget` کادرِ ورودی را با
  `hasAnyConnection('AUTH')` disable می‌کند و آن تابع همین
  `@appkit/connections` را می‌خواند. حالا گزاره‌ی `isConnectionKey()` **شکل‌محور**
  است: هر کلیدِ `@appkit/*` وضعیتِ اتصال است مگر cache یا ترجیحِ کاربر باشد
  (`*_cache`، `portfolio`، `token_price`، `recent_emails`، `latest_version`،
  `preferred_account_types`) و `@appkit/connection_status` **ارزش‌محور** است:
  فقط مقدارِ `'connected'` وضعیت است — چون AppKit هر بوتِ تمیز آن را با
  `'disconnected'` بازنویسی می‌کند و اگر آن را هم «اتصال» بشماریم، گزارشِ سلامت
  هر صفحه‌ی ساکتی را یتیم اعلام می‌کرد. `@appkit-wallet/*` مثل قبل هرگز پاک
  نمی‌شود (نشستِ احرازِ کیفِ جاسازی‌شده است).

- **۲. نمونه‌ی زنده‌ی AppKit هم کثیف می‌مانَد — علتِ «کادرِ ایمیل کار نمی‌کند».**
  پاک‌سازیِ storage به نقشه‌ی **در-حافظه‌ی** `ConnectionController` of نمونه‌ای
  که همین حالا روی صفحه است نمی‌رسد؛ اگر فروپاشیِ نشستِ قبلی در مسابقه‌ی
  ۴ ثانیه‌ایِ `forget()` ببازد، همان نقشه تا آخرین عمرِ صفحه یک اتصالِ AUTH را
  نشان می‌دهد و کادرِ ایمیل از همان لحظه‌ی اول غیرفعال رندر می‌شود. حالا
  `getAppKit({ fresh })` — که فقط `open()` وقتی **هیچ لاگینی ادعا نشده**
  می‌فرستد — سه لایه را جارو می‌کند: storage را همان اول پاک می‌کند؛ با
  `hasAnyConnection('AUTH')` و `getIsConnectedState()` کثیفیِ در-حافظه را
  می‌سنجد؛ و اگر کثیف بود، نمونه را (با سقفِ ۴ ثانیه) retire و از نو می‌سازد تا
  بوتِ تازه فقط وضعیتی را ببیند که چیزی کهنه توصیفش نمی‌کند. وقتی لاگینی
  ادعا شده (restore) هیچ‌چیز پاک نمی‌شود — آن کلیدها مالِ کیفی است که به کاربر
  بدهکاریم. `forget()` هم بعد از disconnectِ محدودکرده، خودش
  `purgeConnectionKeys()` را صدا می‌زند تا دیسکانکتی که sign-out آن دیر می‌رسد
  هم وضعیتی تمیز به بوتِ بعدی بدهد.

- **۳. `modal.open()` بی‌کران بود و provider صفرِ بی‌پیام — علتِ «۷۵ ثانیه چرخید و هیچ».**
  در AppKit 1.8.19، متدِ `open()` پیش از ساختنِ نمایِ Connect روی
  `ApiController.prefetch()` منتظر می‌ماند (فن‌ outی از تماس‌های Explorer API)؛
  روی شبکه‌ی موبایلِ کند یک تماسِ معلق کلِ جریان را نگه می‌داشت و trace همان
  «۷۵ ثانیه تا تایم‌اوت» را نشان می‌داد. حالا `open()` خودمان کرانی دارد
  (`TIMEOUT.emailModalOpen: 20s`) که جریان را به «منتظرِ حسابِ کاربر ماندن»
  آزاد می‌کند، نه اسیرِ prefetch. و وقتی آدرس می‌آید ولی provider نه، به‌جای
  نشان‌دادنِ یک کیفِ «وصلِ» قلابی که فقط موجودی دارد و یک بایت هم امضا
  نمی‌کند، حالا: اول provider را از خودِ auth connector می‌گیریم
  (`authConnectorProvider()` — همان `W3mFrameProvider`، همان شیء EIP-1193 که
  آداپتور ethers می‌راند؛ «تیک سبز و بعد هیچ» روی WebViewهای کند دقیقاً از
  عقب‌ماندنِ یک tick همین‌جا بود)، و اگر واقعاً نیامد، پیامِ صادقِ
  `wallet.emailProviderPending` نشان می‌دهیم: «ورود انجام شد؛ یک بار دیگر
  تلاش کنید یا اپ را باز کنید — اتصال شما از دست نرفته». مارکر می‌ماند تا
  restore در بوتِ بعدی وصلش کند.

**گزارشِ سلامت** حالا دو چیزِ تازه می‌گوید: پرابِ
`GET /appkit/v1/project-limits` — چون خودِ Reown وقتی پروژه از سقف
`starter` (MAU یا RPC) بگذرد کادرِ ایمیل را **خودش** غیرفعال می‌کند
(`hasExceededUsageLimit`) و هیچ پرابِ رله‌ای آن را توضیح نمی‌داد؛ و دو فکتِ
storage (`connectionStatus` و `storedConnectors`) — تا باقی‌مانده‌ای که لاگینِ
بعدی را می‌شکند، در خودِ گزارش نام‌دار باشد.

**تست:** `npm run test:wallet-connection` → ۲۳۸ گزاره ✅ (۱۳ گزاره‌ی تازه: کلیدهای
1.8.19، گزاره‌ی ارزش‌محورِ connection_status، بقاِ cache/نسخه در purge، و فکت‌های
تازه‌ی گزارش). بیلد ✅.

# ۲۰۲۶-۰۹-۱۷ (۵) — «والت باز می‌شود اما وصل نمی‌شود و برگشت به سایت ما نمی‌آید»

> گزارشِ واقعیِ دستگاه (اندروید ۱۰ / Chrome ۱۵۱ روی `https://fbtswap.ir`):
> `relay_preflight_open` → `relay_try` → `provider_ready` → `init` →
> `metadata_repaired` → `appkit_links_applied` → `deeplink_opened_sync` →
> **`connect_failed_relay` با `n: 30772`**. و بعد از دکمهٔ برگشت، مرورگر روی
> `trust://wc?uri=wc%3A…` می‌ماند، نه روی دامنهٔ خودمان — در حالی که همان گزارش
> می‌گوید سوکتِ رله در **۱۰۴۲ms** باز شده و اسکنِ کیوآر از اول کار می‌کرده.

سه علتِ مستقل، هر سه در یک مسیر؛ هیچ‌کدام ربطی به رله نداشتند.

- **۱. فیوزِ ۲۰ ثانیه‌ایِ `connect()` — علتِ «وصل نمی‌شود».** یک رفت‌وبرگشتِ
  موبایلی (لمس → جابه‌جایی اپ → بازکردن قفل والت → خواندن → تأیید → انتشار روی
  رله) در ۲۰ ثانیه جا نمی‌شود. در این گزارش کاربر ۴.۵ ثانیه بعد از `init` روی
  تراست زده و تلاش ۲۰ ثانیه بعد — **در حالی که داخل تراست داشت صفحهٔ تأیید را
  می‌خواند** — کشته شده: `release()` پرووایدر را پایین کشیده، سوکت بسته شده و
  تأییدی که کاربر یک لمس بعد می‌زند به رله‌ای می‌رود که دیگر کسی گوش نمی‌دهد.
  حالا `TIMEOUT.connect` سه عدد دارد: بودجهٔ رویِ صفحه (`75_000`)، بودجه‌ای که
  در لحظهٔ تحویلِ جفت‌سازی به والت داده می‌شود (`connectInWallet: 240_000`)، و
  سقفِ سختی که از عمرِ خودِ URI جلو نمی‌زند (`connectHardCap: 300_000` =
  `PAIRING_TTL_MS`). کران جدید (`pauseBound` در `timing.js`) **هنگامی که سند
  مخفی است متوقف می‌شود**: وقتی کاربر داخل اپ والت است، ما منتظر اوییم نه او.
  سقفِ سخت هیچ‌وقت pause نمی‌شود، پس تلاشی که کاربر برنمی‌گردد هیچ‌وقت آویزان
  نمی‌ماند.
- **۲. طبقه‌بندیِ غلط — علتِ چهار روز پیگردِ VPN.** `isRelayError()` روی
  `/timeout/i` هم match می‌شد، برای همین `WC_CONNECT_TIMEOUT` به
  `WC_RELAY_UNREACHABLE` ترجمه می‌شد و رابط می‌گفت «رله روی این شبکه بسته است؛
  VPN را روی کل دستگاه فعال کن». رله همان لحظه URIای را صادر کرده بود که والت
  با آن باز شده بود — یعنی دقیقاً کار می‌کرد. حالا کرانِ خودمان کدِ مستقلِ
  `WC_TIMEOUT` دارد (پیامِ `wallet.wcTimeout`) و فقط شکستِ واقعیِ سوکت
  (`WC_INIT_TIMEOUT` / خطای شبکه) «رله» نامیده می‌شود. `clearRelayCache()`
  هم دیگر روی تایم‌اوت صدا زده نمی‌شود: اندازه‌گیریِ سالم با یک تایم‌اوت بی‌ربط
  باطل نمی‌شود.
- **۳. تبِ مردهٔ پشتِ دیپ‌لینک — علتِ «برگشت به سایت ما نمی‌آید».** کروم والت را
  از یک `_blank` با طرحِ بومی درست launch می‌کند، اما تبِ جدیدی که ساخته روی همان
  `trust://wc?uri=…` می‌ماند؛ دکمهٔ برگشت کاربر را به همان تب می‌آورد و نتیجه‌گیری‌اش این است که «هیچ‌وقت برنمی‌گردم به fbtswap.ir». دو اصلاح:
  - `tryOpen()` دیگر `noopener` نمی‌فرستد. `window.open` با `noopener` طبقِ
    مشخصات `null` برمی‌گرداند، پس `Boolean(null)` یعنی «شکست» و مسیرِ
    `intent://` همیشه رد می‌شد و نوبت به همان لینکِ خامِ بومی می‌رسید — تنها
    URLای که تبِ مرده جا می‌گذارد. حالا `noreferrer` می‌ماند (دسته) و
    `noopener` نمی‌آید (فقط برای https، جایی که پنجره واقعاً یک مقصد است).
  - تبِ بازشده برای یک **طرحِ بومیِ خام** بعد از `TIMEOUT.handoffClose`
    (۲۵۰۰ms) بسته می‌شود. مسیرِ `intent://` را Chrome خودش رهگیری می‌کند (چیزی
    جا نمی‌ماند؛ اگر والت نصب نباشد همان تب صفحهٔ `browser_fallback_url` را
    نشان می‌دهد که کاربر لازمش دارد) و مسیرِ https هم مقصد است، نه پرتاب.
- **۴. تشخیص.** گزارشِ سلامت حالا `handoff` را هم چاپ می‌کند: `channel`،
  `android`، `ios`، `webview`، `telegram`، `intentCapable`، `javaBridge`. و
  ردیابی مسیرِ تحویل را نام می‌برد (`handoff_intent` / `handoff_native` /
  `handoff_universal` / `handoff_java_bridge`) تا گزارشِ بعدی بگوید جفت‌سازی از
  **کدام پله** بیرون رفته، نه فقط اینکه بیرون رفته. پنلِ سلامت یک ردیفِ
  «کانالِ تحویل» دارد.
- **۵. ایمیل: مودال پیش از آماده‌شدنِ AppKit باز می‌شد.** `createAppKit()`
  همزمان برمی‌گردد و `initialize()` را در پس‌زمینه شروع می‌کند
  (`this.readyPromise = this.initialize(options)`) و `open()` هیچ‌وقت منتظرش
  نمی‌ماند. هرچه ایمیل لازم دارد **در انتهای** همان boot ساخته می‌شود:
  `fetchRemoteFeatures()` → `createAuthProvider()` → iframeِ
  `secure.walletconnect.org`. یعنی فیلدِ ایمیل رسم می‌شد (از `features`ای که خودمان
  re-assert می‌کنیم) در حالی که ارائه‌دهنده‌ای که باید جوابش را بدهد هنوز وجود
  نداشت — همان «تیک سبز می‌آید اما هیچ اتفاقی نمی‌افتد». `getAppKit()` حالا
  منتظر `readyPromise` می‌ماند (کران‌دار، و در تایم‌اوت هم مودال را باز می‌کند تا
  یک APIیِ مسدود دکمهٔ Connect را نمیراند) و نتیجه را با `email_appkit_ready*`
  ثبت می‌کند.
- **تست:** بخش‌های ۱۳ و ۱۴ به `test/walletconnect-stack-probe.mjs` اضافه شد —
  توقف/ادامه/سقفِ سختِ `pauseBound`، اینکه بودجهٔ درون-والت از یک رفت‌وبرگشتِ
  موبایلی بزرگ‌تر است، اینکه WebView هیچ‌وقت `intent://` نمی‌گیرد، اینکه
  `window.open` دسته را نگه می‌دارد، و اینکه تبِ طرحِ بومی بسته می‌شود و تبِ
  `intent://` نه. `npm run test:wallet-connection` سبز: **۲۲۰** ادعا + ۴ تستِ
  پنل + ۸ تستِ شیتِ مانت‌شده.

---

# ۲۰۲۶-۰۹-۱۷ (۴) — بودجهٔ probe رله: ۵ ثانیه لبِ مرز بود، حالا ۸ ثانیه

> اندازه‌گیریِ واقعیِ دستگاه (Samsung Internet 30 / اندروید ۱۰، دیتای موبایل):
> `wss://relay.walletconnect.org` → ok در **۴۳۴۴ms** · `wss://relay.walletconnect.com`
> → ok در ۳۰۷۸ms · همان میزبان‌ها روی https → ۲۲۰ms و ۳۰۶ms. یعنی هزینه،
> handshake یِ wss است، نه شبکه.

- `TIMEOUT.relayProbe` در `src/lib/wc/config.js` از `5_000` به `8_000` رفت. سوکتِ
  سالمِ همان دستگاه ۸۷٪ بودجهٔ ۵ ثانیه‌ای را می‌خورد؛ یک تأخیرِ کوچک verdict را به
  `TIMEOUT` می‌بُرد و رابط به کاربر می‌گفت «رله روی این شبکه بسته است» — منفیِ
  کاذبی برای شبکه‌ای که جفت‌سازی رویش کار می‌کند. مصرف‌کننده‌ها (`probeRelay` و
  `measureRelay` در `relay.js`) پارامترِ پیش‌فرض را از همین ثابت می‌خوانند، پس جای
  دیگری تغییر نکرد؛ کامنتِ ثابت تاریخ/دستگاه/۴۳۴۴ms و هشدارِ «دوباره پایین نیاورید»
  را نگه می‌دارد.
- `TIMEOUT.initFirst: 8_000` اندازه‌گیری و بررسی شد و **بالا نرفت**: با سوکتِ
  ساختگیِ ۴۳۴۴ms، `EthereumProvider.init()` در ۱۵ms برمی‌گردد و **صفر** سوکت
  می‌سازد (`Relayer.init()` همان `transportOpen()` را بدون `await` صدا می‌زند و با
  نبود topic اصلاً سوکتی باز نمی‌کند)، پس هزینهٔ handshake در `init()` نیست؛ در
  `connect()` است که بودجهٔ خودش (`TIMEOUT.connect = 20_000`) را دارد. ۸ ثانیه برای
  «import + مقداردهیِ اولیه» گشاد است.
- `TIMEOUT.relayCacheTtl: 90_000` دست‌نخورده ماند و با مقدارِ تازه هم معقول است:
  TTL ≥ ۱۰× بودجهٔ probe، verdictِ کش‌شده فقط ترتیبِ تلاش را عوض می‌کند (توصیه‌ای،
  نه تصمیم‌گیر)، و شکستِ واقعیِ رله `clearRelayCache()` را صدا می‌زند.
- **تست:** ادعای تازه در بخشِ ۶ `test/walletconnect-stack-probe.mjs` —
  `TIMEOUT.relayProbe > 4344ms` با حداقلِ ۱.۵× فاصله، تا این کران دوباره بی‌صدا به
  لبِ مرز برنگردد. `npm run test:wallet-connection` سبز: **۱۹۹** ادعا + ۴ تستِ پنل.
...
> `✅ تنظیمات پروژه — email=false socials=0` · `❌ رلهٔ WalletConnect — SOCKET_ERROR`.
> داشبورد در همان لحظه (خوانده‌شده از API زنده) `social_login` را روشن با هشت نام
> می‌داد. ⇒ هر دو عدد، غلطِ **ابزار** بودند، نه شبکه — و یک عددِ غلطِ مطمئن
> بدتر از هیچ عددی است، چون همان است که به پشتیبانی گفته می‌شود.

- **خطای اول (payload):** `summarizeProjectConfig()` مقدارِ `features.social_login`
  را می‌خواند؛ ولی `GET /appkit/v1/config` آرایه می‌دهد و خوانندهٔ خودِ SDK
  `ConfigUtil.getApiConfig()` است: `apiProjectConfig?.find((f) => f.id === id)`.
  روی آرایه، `.social_login` همیشه `undefined` است ⇒ پنل **همیشه**
  `email=false socials=0` چاپ می‌کرد، حتی وقتی داشبورد ایمیل و ۷ سوشال را روشن
  داشت. حالا شکلِ آرایه‌ای با قاعدهٔ خودِ SDK خوانده می‌شود (`isEnabled` +
  `config.includes('email')`؛ socials = همان لیست منهای `email`)، شکلِ قدیمیِ
  شیء-نقشه‌ای هم پذیرفته می‌شود، `config: null` به‌عنوان «لیست پس‌گرفته‌شده»
  گزارش می‌شود (نه لیستِ خالی)، و `shape` کنارش می‌آید تا تغییرِ بعدیِ payload
  به‌صورت `shape:"none"` دیده شود نه «ایمیل خاموش است». ردیفِ پنل هم لیستِ
  socials را چاپ می‌کند، نه فقط تعدادشان.
- **خطای دوم (رله):** پنل فقط `wss://relay.walletconnect.com` را می‌پرسید و
  نتیجه را زیر برچسبِ «رلهٔ WalletConnect» چاپ می‌کرد — درحالی‌که در
  `@walletconnect/core@2.25.0`، `RELAYER_DEFAULT_RELAY_URL` مقدار
  `wss://relay.walletconnect.org` است (تنها رشتهٔ `wss://` آن باندل)، FAQ رسمیِ
  Reown هم برای «رلهٔ پیش‌فرض بسته است» همان `.org` را می‌دهد، و `relayUrl` در
  هیچ‌جای `@reown/appkit@1.8.19`/controllers/adapter وجود ندارد (تنها تنظیمِ رلهٔ
  این اپ همان `EthereumProvider.init({ relayUrl })` است). یعنی `.com` میزبانِ
  تاریخی است و اپ ما آن را **اول** می‌گذاشت و فیوزِ ۸ ثانیه‌ای را روی همان
  override خرج می‌کرد.
- **فیکسِ رله:** `WC_RELAY_URLS` با ترتیبِ خودِ SDK (`.org` اوّل، `.com` دوم)؛
  پنل **هر دو** میزبان را می‌سنجد، هر کدام با دو در (سوکت + HTTPS) و با زمانِ
  خودش؛ `probeRelay` حالا `ms` و `closeCode` را هم می‌دهد (۱۰۰۶ دیگر به‌عنوان
  «میزبان رد کرد» جا زده نمی‌شود، کدِ واقعیِ رله مثل ۳۰۰۰ قابلِ تشخیص است)؛ و
  حکم از یک تابعِ خالص می‌آید: `OPEN` / `WS_REFUSED` (HTTPS رسید ولی ارتقای
  WebSocket بسته است) / `UNREACHABLE` / `TIMEOUT` / `NO_WEBSOCKET` /
  `NO_MEASUREMENT`. جملهٔ حکم و «سه مسیری که به رله نیاز ندارند» (کیف پول
  درون‌برنامه‌ای، کیف پول تزریق‌شده از مرورگرِ خودِ کیف پول، ایمیل/سوشال که
  ناقله‌اش فریمِ امن است نه رله) در fa/en/ar اضافه شد.
- **تست:** `test/wallet-health-probe.mjs` از ۳۶ به **۶۲** ادعا رفت: پاسخِ زندهٔ
  همین پروژه به‌عنوان fixture داخلِ تست آمده (`features` آرایه‌ای،
  `social_login` روشن با هشت نام)، شکلِ قدیمی و لیستِ پس‌گرفته‌شده و payloadِ
  خراب جدا سنجیده می‌شوند، هر پنج حکمِ رله با لبه‌های شبیه‌سازی‌شده قفل شده‌اند،
  و سناریوی خودِ گزارش («یک میزبان بسته + میزبانِ پیش‌فرض باز») باید `OPEN`
  بدهد نه «رله بسته است». `test/wc-timeout-probe.mjs` هم با ترتیبِ جدید
  هم‌راستا شد. گروهِ والت: **۴۱۹ ادعا، ۰ شکست**؛ `npx vite build` سبز.
- **مستند:** `docs/WALLET-HEALTH-INSTRUMENT-FIX-FA.md` (شواهد + کاری که کاربر
  باید بکند) و تصحیحِ بخش‌های ۱/۵/۷ در `docs/WALLET-AUDIT-ROOT-CAUSE-FA.md`.

# ۲۰۲۶-۰۹-۱۶ (۲) — بازگشتِ ریدایرکت در ورود ایمیل/سوشال: مارکرِ بوت پیش از باز شدنِ مدال claim می‌شود

> گزارش: «ایمیل را تأیید می‌کنم، برمی‌گردد سایت، ولی کیف پول وصل نمی‌شود. اگر
> یک بار دیگر «ایمیل و ورود با سوشال» را بزنم، همان‌دم و بدون باز شدنِ مودال
> وصل می‌شود.» Dashboard روشن است و Allowed Domains هر دو مبدا را دارد — پس
> مشکل از کد ماست، نه از پیکربندی.

- **ریشه — یک مسابقهٔ بین دو سند:** فلوی ایمیل/سوشال روی موبایل (و هر
  مرورگری که OTP/OAuth را به ناوبریِ تمام‌صفحه می‌دهد) ریدایرکت‌محور است:
  تأیید، مرورگر را از `fbtswap.ir` بیرون می‌برد و برگشتنش یک **سندِ تازه**
  است. آن سند هیچ‌کدام از `subscribeAccount`/`subscribeState`های صفحهٔ قبل را
  ندارد، درحالی‌که نشستِ AppKit گرم است. تنها چیزی که از آن سفر برمی‌گردد
  مارکرِ بوت (`fbt_email_social_connected`) بود — و مارکر **فقط** داخل
  `attachEmailProvider()` و بعد از attachِ در-صفحه‌ای نوشته می‌شد؛ یعنی در
  همین فلوی ریدایرکت‌شده هرگز نوشته نمی‌شد. در نتیجه دروازهٔ cold-start
  (`hasEmailSocialMarker()`) به `restoreEmailSocial()` نمی‌رفت و سراغ
  `restoreWcSession()` می‌رفت: سشنِ گرم برای مدلِ ما **نامرئی** می‌ماند.
  علامتِ بالینیاش همان «یک بار دیگر بزن» بود — دوباره‌زدن، نشستِ گرم را در
  همان صفحهٔ زنده attach می‌کرد و کار می‌کرد.
- **راستی‌آزمایی از سورسِ واقعی `@reown/appkit@1.8.19`:** `subscribeState` روی
  valtio است و هم‌زمانِ subscribe emit نمی‌کند — پس این «eager-settle» نبود که
  فلوی در-صفحه‌ای را زود ببندد؛ `onAuthProviderConnected` هم اول `syncAccount`
  را اجرا می‌کند، یعنی مسیرِ in-page سالم بود و فقط **بازگشتِ ریدایرکت**
  پوشش داده نشده بود.
- **فیکس، دو خطِ رفتاری:** ۱) `connectEmailSocial()` مارکر را در همان ابتدا
  claim می‌کند — بعد از teardown تک‌کیفِ خودش (که هم‌زمان مارکر را پاک
  می‌کند، پس پیش از آن نوشتن بی‌اثر می‌ماند) و **پیش از** `getEmailSocialAppKit()`
  و `modal.open()`؛ ۲) هر پایانی که اتصالی از آن درنیاید با
  `rollbackEmailSocialMarker(modal)` همان claim را پس می‌دهد: لغوِ مدال،
  `attachEmailProvider` خطاداده یا بازگشتِ false، و catchِ بیرونی (شکست در
  ساختنِ نمونه/چانک). rollback **صادقانه** است، نه کور: اگر خودِ AppKit هنوز
  حسابِ متصل گزارش کند، مارکر می‌ماند — همان حالتی که ریکاوریِ cold-start
  دقیقاً برای آن ساخته شده.
- **مرزهای خودپاک‌کننده دست‌نخورده‌اند، و همین claim زودهنگام را بی‌ریسک
  می‌کند:** `restoreEmailSocial()` اگر تا ۸ ثانیه حسابی نبیند مارکر را می‌شوید
  (پس لوپِ مرده نمی‌شود) ولی در خطا (آفلاین/چانکِ مسدود) نگهش می‌دارد؛
  `disconnect()` و افکت `[mode]` روی اتصالِ کیفِ دیگر همان‌طور که قبلاً
  پاک می‌کردند پاک می‌کنند؛ نوشتنِ `attachEmailProvider` هم سرِ جایش ماند تا
  یک اتصالِ اثبات‌شده claim را نو کند. منطقِ جریانِ WalletConnect، تعهدِ
  آتش‌بسِ features، `emailSocialOptions` و فلگ `emailModalActive` هیچ‌کدام
  تغییر نکردند؛ رمزِ جدیدی هم لازم نیست — فقط همان Project ID عمومی.
- **PWA:** shell cache از v13 به v14 رفت. این رفتار **داخل** خودِ shell است و
  در نصبی که هنوز v13 را دارد همان باگ قدیمی byte-for byte سرو می‌شود؛
  تغییرِ نامِ کش، همان نصب‌ها و آپدیتِ درجای APK را یکسان evict می‌کند.
- **تست:** `test/email-social-probe.mjs` از ۲۶ به **۳۹** ادعا رفت: ادعاهای
  جدیدِ source-strip جایگاهِ claim را می‌سنجند (داخل `connectEmailSocial`،
  بعد از teardown، پیش از `getEmailSocialAppKit(` و `modal.open()`)، هر سه
  نقطهٔ rollback را جدا جدا پین می‌کنند، و بقای هر مرزِ خودپاک‌کننده (۸ ثانیه،
  disconnect، افکت `[mode]`) را قفل می‌کنند؛ `rollbackEmailSocialMarker` هم
  رفتارش روی Storage استتار و مودالِ دروغگو/پاسخ‌ندهاده سنجیده می‌شود. ادعاهای
  جدید با mutation راستی‌آزمایی شدند: با جابه‌جا کردنِ claim به back-at-attach،
  با حذفِ هر یک از rollbackها و با برداشتنِ شستنِ ۸ ثانیه‌ای، همان ادعا می‌شکند. هر شش probe والت (wc-connect / wc-wallets / wc-deeplink /
  wc-uri-hygiene / wc-pairing-surface / walletconnect-wiring) بدون رگرسیون
  سبزند، `npx vite build` سبز است و چانکِ lazy AppKit همچنان بیرونِ گرافِ
  استاتیکِ entry مانده است.
- **مستند:** `docs/EMAIL-SOCIAL-LOGIN-FA.md` بخشِ «رفتارِ مرزبندی» یک subsection
  گرفت: چرا زمانِ نوشتنِ مارکر مالِ «قبل از مودال» است و جدولِ مرزهای پس‌دادن.
...
- **رشته‌ای که در خطا دیده می‌شود خودِ pairing URI است، نه لینکی که ما ساختیم.**
  با اجرای خودِ SDK (`@walletconnect/core`، در پروسهٔ جداگانه و با رلهٔ خودکارِ
  خاموش) `pairing.create()` عیناً همین شکل را می‌دهد:
  `wc:<۶۴ کاراکتر هگز>@2?expiryTimestamp=…&relay-protocol=irn&symKey=…`
  (ترتیبِ پارامترها الفبایی است چون `formatUri()` کلیدها را sort می‌کند).
- **`&amp;` در آن رشته کشنده است، نه تزئینی.** `pairing.pair()` روی همان رشته
  با `&amp;` می‌گوید `Missing or invalid. pair() uri#relay-protocol` (چون
  `parseUri()` سخت‌گیر نیست و بی‌صدا `symKey` و `relay-protocol` را از دست
  می‌دهد) و روی نسخهٔ ترمیم‌شده بدون خطا جفت می‌شود. `&amp;` را هیچ‌جای SDK
  تولید نمی‌کند؛ اثرِ عبورِ URL از یک سطحِ HTML است.
- **متنِ «Invalid Url:» از کدِ ما یا SDK نیست.** در هیچ‌کدام از بسته‌های
  `@walletconnect/*` و `@reown/*` و Capacitor چنین رشته‌ای وجود ندارد (هر دو
  حالتِ حروف بررسی شد). این تاریخِ خطای مقصد است: مرورگر/WebView که آدرسی با
  طرحِ ناشناخته (`wc:`) را باز نمی‌کند و خودِ آدرس را زیرش چاپ می‌کند — همان
  کلاسی که در RainbowKit #1531 برای مینی‌اپِ تلگرام گزارش شده است.
- **حفرهٔ باقی‌مانده، دقیقاً همین بود:** `decideWalletOpen()` فقط URLهایی را
  برمی‌داشت که بارِ جفت‌شدن داخلِ یک لینکِ کیف‌پول دارند (`uri=`)، و یک `wc:`ِ
  خالی را عمداً `{action:'pass'}` می‌کرد — یعنی دست‌نخورده به WebView؛ و هیچ‌جا
  `&amp;` ترمیم نمی‌شد. فیکسِ قبلی لینک‌های کیف‌پول را درست کرده بود، نه این دو.
- **`src/lib/wcWallets.js`:** `looksLikePairingUri()`، `repairPairingUri()`
  (هر سه شکلِ entity: `&amp;` / `&#38;` / `&#x26;`؛ روی رشتهٔ سالم بایت‌به‌بایت
  no-op) و رجیستریِ کیف‌پولِ لمس‌شده (`rememberTappedWallet()` /
  `lastTappedWallet()` — فقط `key`، بدونِ هیچ دادهٔ نشست).
- **`src/lib/wcDeepLink.js`:** `repairPairingInUrl()` (ترمیمِ داخلِ پارامترِ
  `uri=`، چه percent-encode شده چه نشده؛ لینکِ سالم دست‌نخورده)؛ و
  `decideWalletOpen()` حالا اول ترمیم می‌کند و یک `wc:`ِ خالی را به لینکِ httpsِ
  همان کیف‌پولی تبدیل می‌کند که کاربر لمس کرده است — و اگر کیف‌پولی لمس نشده
  باشد هیچ لینکی نمی‌سازد (مسیرِ QRِ دسکتاپ دست‌نخورده می‌ماند).
- **`src/lib/wcAppKitPatch.js`:** همان‌جا که SDK لینک می‌سازد
  (`ConnectionControllerUtil.onConnectMobile`) کیف‌پولِ در دستِ تحویل ثبت
  می‌شود — تنها راهِ صادقانهٔ دانستنِ «کدام کیف‌پول».
- **`src/context/WalletContext.jsx`:** رویدادِ جدیدِ `deeplink_uri_repaired`
  کنارِ `deeplink_rewritten` / `deeplink_opened` / `deeplink_open_failed`.
  بودنِ این رویداد یعنی URLِ آسیب‌دیده از دستِ ما گذشته (و ترمیم شده)؛
  نبودنش یعنی آن URL را چیزِ دیگری باز کرده است.
- **`test/wc-uri-hygiene-probe.mjs`** (۳۲ بررسی، بخشِ `0c-7` در `npm test`):
  شکلِ URIِ SDK در پروسهٔ فرزند، رد شدنِ نسخهٔ `&amp;`دار توسط خودِ `pair()`،
  ترمیمِ entityها و idempotent بودنش، بی‌اثری روی لینکِ سالم، `%26` (نه
  `&amp;`) در لینکِ ساخته‌شده، `wc:`ِ خالی → لینکِ کیف‌پولِ لمس‌شده، و
  دست‌نخورده ماندن وقتی کیف‌پولی لمس نشده. ۳۰ بررسیِ لینکِ کیف‌پول و ۳۴ بررسیِ
  تحویلِ دیپ‌لینک هم دست‌نخورده پاس می‌شوند.
- **یادآوریِ عملی:** هر pairing URI حدوداً **۵ دقیقه** زنده است؛ اگر همان متن یا
  اسکرین‌شاتِ قبلی دوباره باز شود، حتی URIِ سالم هم کار نمی‌کند و باید دکمهٔ
  «بازکردن» دوباره زده شود. متنِ کامل: `docs/WALLETCONNECT-INVALID-URL-URI-FA.md`.

# ۲۰۲۶-۰۹-۱۵ (۳) — «ارور دیپ‌لینک» با زدنِ «بازکردن» واقعاً رفع شد (علت در خودِ SDK)

- **علتِ دقیق، این بار با اجرای خودِ SDK اندازه‌گیری شد نه با حدس: کیف‌پولی که
  کاربر روی موبایل لمس می‌کند مالِ ما نیست.** `customWallets` فقط در گروهِ
  «custom» دیده می‌شود؛ ردیفِ WalletConnect روی موبایل مستقیم به صفحهٔ
  **All wallets** می‌رود و آن صفحه از پاسخِ `api.web3modal.org` رندر می‌شود
  (`ApiController.state.wallets`). با آیدی همین پروژه، همان پاسخ می‌گوید
  `Trust Wallet → { mobile_link: 'trust://', link_mode: null }` و
  `MetaMask → { mobile_link: 'metamask://', link_mode: null }`. بدونِ
  `link_mode` تابعِ `CoreHelperUtil.formatNativeUrl()` مقدارِ
  `redirectUniversalLink` را `undefined` می‌گذارد، پس شرطِ
  `experimental_preferUniversalLinks && redirectUniversalLink` هیچ‌وقت برقرار
  نمی‌شود و AppKit **همان طرحِ اختصاصی** را باز می‌کند:
  `window.open('trust://wc?uri=…','_self')` — یعنی یک طرحِ ناشناس برای WebView،
  که دقیقاً همان صفحهٔ خطایی است که گزارش شد. پس خطِ
  `experimental_preferUniversalLinks: true` به‌تنهایی کافی نبود: چیزی برای
  «ترجیح دادن» وجود نداشت.
- **لایهٔ اول: `src/lib/wcAppKitPatch.js`** — تنها جایی که SDK از شیءِ کیف‌پول
  لینک می‌سازد (`ConnectionControllerUtil.onConnectMobile`) پوشش داده می‌شود و
  شیءِ کیف‌پول پیش از رسیدن به آن `link_mode` لینکِ https همان کیف‌پول را
  می‌گیرد (`withLinkMode()` در `src/lib/wcWallets.js`؛ شیءِ اصلی mutate نمی‌شود).
  این کار «بهترین تلاش» است و idempotent: اگر روزی نامِ تابع عوض شود `false`
  برمی‌گرداند و لایهٔ دوم کار را می‌کند.
- **لایهٔ دوم: `src/lib/wcDeepLink.js`** — در تمامِ مدتِ یک جفت‌شدن، `window.open`
  (همان تابعی که `CoreHelperUtil.openHref()` صدا می‌زند) پوشش داده می‌شود: هر
  دیپ‌لینکِ کیف‌پول پیش از بیرون رفتن به لینکِ https همان کیف‌پول تبدیل و از
  کانالِ خودمان تحویل می‌شود (`openWalletLink()` در `src/lib/browser.js`) —
  در تلگرام با `WebApp.openLink`، در اپ با Android Custom Tabs (مرورگرِ واقعی؛
  پس WebView صفحهٔ خطا نشان نمی‌دهد و App Links لینک را به کیف‌پول می‌سپارد)، و
  در وب با همان `window.open`. هرچه بعد از `scheme://` می‌آید بایت‌به‌بایت کپی
  می‌شود تا URI دوباره encode نشود. پل فقط URLهای «دارای بارِ جفت‌شدن»
  (`uri=`) را برمی‌دارد؛ لینک‌های خود AppKit (Get a wallet، لینک‌های فروشگاه)
  دست‌نخورده رد می‌شوند. نصب در `connectWalletConnect()` **قبل از**
  `wc.connect()` و برداشتن در `finally` (هیچ ری‌رایتی برای بقیهٔ نشست نمی‌ماند).
- **تست** — `test/wc-deeplink-probe.mjs` (۳۴ ادعا، داخل `npm test`) روی SDK
  نصب‌شده: قبل از patch → `trust://wc?uri=…`، بعد از patch →
  `https://link.trustwallet.com/wc?uri=…`؛ متامسک هم لینکِ خودش را می‌گیرد؛
  کیف‌پولِ ناشناس دست‌نخورده می‌ماند (هیچ لینکی از خودمان ساخته نمی‌شود)؛
  و پل: rewrite، رد شدنِ لینک‌های غیرکیف‌پول، عدم بازگشت به `window.open`
  (recursion)، و uninstall دقیق. `npm run build` سبز.
- **آنچه فقط از دست خودت برمی‌آید** — تا deploy جدیدِ سایت و نصبِ APK تازه،
  همان خطا روی نسخهٔ قدیمی باقی می‌ماند: کدِ داخل باندلِ سایت و داخل APKِ روی
  گوشی هنوز قدیمی است.

# ۲۰۲۶-۰۹-۱۵ (۲) — هیدر و فوترِ خودِ موبایل شیشه‌ای شد و هم‌رنگ سایت

- **همان هیدری که میزان شارژ را نشان می‌دهد (نوار وضعیت) و فوتری که دکمه‌های موبایل (بازگشت/خانه/تازه‌ها) است، حالا شیشه‌ای است و با تم سیاه و سفید هم‌رنگ سایت می‌شود.** این دو نوار را اندروید می‌کشد، نه صفحه — تنها جایی که قابل تغییر است کدِ native بود. `MainActivity` حالا edge-to-edge است (محتوای سایت زیر هر دو نوار رسم می‌شود؛ اپ از قبل `viewport-fit=cover` و safe-area داشت) و رنگ هر نوار رنگِ بومِ همان تم با ۵۵٪ شفافیت است: تم سیاه `#00030F` (مشکی برند) و تم سفید `#F4F5FA`. پس‌زمینهٔ RGB متحرکِ سایت از پشتش دیده می‌شود؛ نتیجه شیشهٔ ماتِ هم‌رنگ سایت است. در اندروید ۱۵+ خودِ سیستم نوارها را کاملاً شفاف می‌کند — همان اثر شیشه‌ای.
- **رنگ آیکون‌ها هم با تم عوض می‌شود** — در تم سیاه ساعت/باتری و دکمه‌ها سفید، در تم سفید مشکی؛ رنگ نوار دیگر هرگز برعکس رنگ سایت پشتِ آن نمی‌شود.
- **همگام‌سازی از سمت وب** — تم (شامل حالت auto) متعلق به لایهٔ وب است، پس `applyTheme` حالا با هر بار اعمال تم، تم را به native می‌فرستد (`window.FBTSystemUI.setTheme` که با `addJavascriptInterface` در MainActivity تعریف شده). اسکریپتِ pre-paintِ `index.html` هم قبل از اولین نقاشی همین کار را می‌کند تا کاربر تم سفید بدونِ فلشِ یک فریمِ مشکی بوت شود، و پیش‌فرضِ فریم‌های اسپلش در `styles.xml` شیشهٔ مشکی است.
- **سمت سایت (مرورگر/تلگرام/PWA آی‌او‌اس) با همان مکانیزم‌های موجود پوشش است و دست نخورده ماند** — `theme-color` برای کُرُم مرورگر، `setHeaderColor`/`setBackgroundColor` تلگرام، و نوار وضعیت `black-translucent` در PWA که پس‌زمینهٔ سایت از پشتش دیده می‌شود (شیشه‌ای است).
- **تست‌ها** — `vite build` سبز، پروبِ تنظیمات ۶۰/۶۰ (همهٔ مسیرهای تغییر تم)، و پروبِ اختصاصیِ سیم‌کشی پل که ثابت می‌کند `setTheme` در هر `applyTheme` (از جملهٔ حلِ `auto`) صدا زده می‌شود و در مرورگر بی‌اثر است.

# ۲۰۲۶-۰۹-۱۵ — رفع خطای «Invalid URL» هنگام اتصال تراست والت (از اپ/سایت)

- **علت دقیق پیدا و رفع شد — مشکل از جفت‌شدن نبود، از آدرسی بود که به گوشی
  می‌دادیم.** دو اشتباهِ جدا در آخرِ یک متر جمع شده بودند. اول: فهرست
  `qrModalOptions.mobileWallets` با شکل `links: { native, universal }` نوشته
  شده بود — شکلِ مودال قدیمی. از
  `@walletconnect/ethereum-provider@2.23` مودالِ داخل SDK **`@reown/appkit`**
  است و `convertWCMToAppKitOptions()` فقط `{ id, name, links }` را کپی می‌کند،
  در حالی که خودِ AppKit هرگز `links` را نمی‌خواند:
  `determinePlatforms()` و `onConnectMobile()` فقط فیلدهای اکسپلورر
  (`mobile_link`، `link_mode`) را می‌بینند. پس آن آیتم‌ها عملاً **هیچ لینکی
  نداشتند** و لمسِ نام تراست والت به صفحهٔ «unsupported» می‌رسید. دوم: لینکی که
  ساخته می‌شد `trust://wc?uri=…` بود؛ یک طرحِ اختصاصی که فقط از مرورگرِ سیستم
  باز می‌شود و داخل WebView (اپ بسته‌بندی‌شده، تلگرام، مرورگرِ خود تراست والت)
  یک طرحِ ناشناس است — همان‌جا WebView می‌گوید **«Invalid URL»** و آدرس را به
  شکل لینک زیرش می‌نویسد. درستش را مستندات خود تراست والت می‌گوید:
  `https://link.trustwallet.com/wc?uri=${encodeURIComponent(uri)}`.
- **یک ماژول تازه و یک منبعِ واحد** — `src/lib/wcWallets.js` جدولِ کیف پول‌های
  معرفی‌شده را نگه می‌دارد و هر دو شکل را تولید می‌کند:
  `appKitCustomWallets()` با `mobile_link` و `link_mode` (فیلدهایی که AppKit
  واقعاً می‌خواند) و `legacyModalWallets()` برای شکل قدیمی. قاعدهٔ چسباندن
  `wc?uri=` دقیقاً همان قاعدهٔ `CoreHelperUtil.formatNativeUrl()` است — فقط
  اسلش اضافه می‌شود، هرگز حذف نمی‌شود (وگرنه `trust://` می‌شد `trust:/wc?uri=`)
  و URI دقیقاً یک‌بار encode می‌شود.
- **`applyAppKitWalletLinks()` بعد از `init()` و قبل از `wc.connect()`** —
  مودالی که خودِ EthereumProvider می‌سازد را با `wc.modal.updateOptions()`
  به‌روز می‌کند: `customWallets` با لینک‌های واقعی، و
  `experimental_preferUniversalLinks: true` تا AppKit به‌جای طرحِ اختصاصی،
  لینکِ **https** را باز کند. همین یک خط «Invalid URL» را از بین می‌برد، چون
  لینکِ https را مرورگر، WebView و خودِ سیستم‌عامل همه می‌فهمند. این کار
  «بهترین تلاش» است: اگر آن متد روزی حذف شود به کنترلرهای AppKit سقوط می‌کند و
  در غیر این صورت فقط رویداد `appkit_links_failed` ثبت می‌شود — هرگز باعث شکستِ
  دکمهٔ Connect نمی‌شود.
- **کیف پول‌های معرفی‌شده دیگر از API اکسپلورر نمی‌آیند** —
  `explorerRecommendedWalletIds: 'NONE'` شد، چون با مقدار قبلی همان کیف پول‌ها
  از پاسخِ `api.web3modal.org` ساخته می‌شدند و روی شبکه‌ای که آن API را فیلتر
  می‌کند غیب می‌شدند. حالا هر سه کیف پول در هر شبکه‌ای با لینکِ درست حاضرند
  (آیکونِ برندشان فعلاً عمومی است؛ یک آیکونِ ساده بهتر از ورودیِ زیبایی است که
  باز نمی‌شود).
- **تعمیرِ متادیتا اصلاح شد** — `populateAppMetadata()` وقتی هاستِ
  `metadata.url` با `window.location.origin` فرق کند آن را بازنویسی می‌کند، و
  داخل APK آن یعنی `https://localhost`. هدفِ این تعمیر `wc.signer.client` است
  (**خودِ SignClient** — نه Core) که همان چیزی است که موتور از آن پیشنهادِ
  نشست را می‌سازد؛ کد قدیم نتیجه را از `wc.signer.metadata` می‌گرفت که وجود
  ندارد، پس ردپا همیشه `metadata_repair_failed` را ثبت می‌کرد و کسی به آن
  اعتماد نمی‌کرد. هر دو شاخهٔ دفاعی (`wc.signer` و `wc.rpc`) مانده‌اند.
- **گارد رگرسیون** — `test/wc-wallets-probe.mjs` با ۳۴ ادعا (داخل `npm test`):
  لینکِ تراست والت بایت‌به‌بایت همان چیزی است که مستندات خودش تجویز می‌کند، URI
  دقیقاً یک‌بار encode می‌شود، هر کیف پول یک لینکِ https دارد، ورودی‌های AppKit
  به `links` وابسته نیستند، و با شبیه‌سازیِ وفادارانهٔ `determinePlatforms()`
  شکل قدیم هیچ پلتفرمی پیدا نمی‌کند (همان باگ) ولی شکل جدید روی موبایل «deep
  link» و روی دسکتاپ «QR» است. ادعاهای قدیمیِ `test/wc-connect-probe.mjs` که
  محلِ متادیتا را اشتباه فرض کرده بودند، با نسخهٔ نصب‌شدهٔ SDK اصلاح شدند
  (اکنون ۱۲۳ ادعا در سه فایل سبز است). جزئیات کامل در
  `docs/TRUST-WALLET-INVALID-URL-FA.md`.

# ۲۰۲۶-۰۹-۱۴ — صفحهٔ لانچ بالا می‌آید (خطای TDZ) + گارد رگرسیون صفحه

- **علت دقیق کرش پیدا و رفع شد** — صفحهٔ لانچ روی هر رندر با
  `ReferenceError: Cannot access 'balanceInfo' before initialization` می‌افتاد،
  پس هیچ‌وقت یک پیکسل هم رسم نمی‌شد و روتر به کارت «این صفحه به مشکل خورد»
  می‌رسید؛ همان چیزی که گزارش شد: «صفحه لانچ میگه با مشکل برخورد و نمیاره».
  ماجرا این بود: کال‌بک `setQuoteMax` نزدیک خط ۲۸۰ اعلام شده بود ولی حالت
  `balanceInfo` حدود ۴۰۰ خط پایین‌تر، پس **آرایهٔ وابستگی** (که در حین رندر و
  به ترتیب خطوط ارزیابی می‌شود) قبل از اجرای `useState` آن را می‌خواند و
  `const` در همان اسکوپ تا خط خودش در حالت TDZ است. حالا حالت قبل از
  مصرف‌کننده‌اش اعلام می‌شود و یک کامنت صریح می‌گوید چرا جابه‌جا کردن این خط
  دوباره صفحه را می‌شکند.
- **یک روزنهٔ دوم هم بسته شد** — رجیستری زنجیره‌ها از `/api/launch/config`
  جای پیش‌فرض‌های داخلی را می‌گیرد و صفحه ردیف‌ها را بدون گارد در رندر
  می‌خواند (`chainMeta.find(...)`، `c.dex.name`). هر پی‌لود با شکل اشتباه —
  صفحهٔ خطای گیت‌وی که هنوز JSON پارس می‌شود، `{"networks":"maintenance"}`،
  ردیف‌های عددی/تهی — مستقیم داخل state می‌رفت و همان کارت کرش را می‌ساخت.
  حالا فقط شکلی که خودمان می‌خواهیم پذیرفته می‌شود و در غیر آن «حقیقت محلی»
  (رژیم offline-first ماژول) سر جایش می‌ماند؛ پی‌لود واقعی سرور (۷ زنجیره)
  بی‌کم‌وکاست پذیرفته می‌شود.
- **چرا هیچ تستی این را نگرفته بود** — `launch-probe.mjs` و
  `launch-solana-probe.mjs` ماژول لانچ را قفل می‌کنند (اعتبارسنجی، بیت‌مپ
  قابلیت‌ها، موتور ریسک، بایت‌های calldata، ماشین حالت) و همه‌شان سبز بودند؛
  عیب داخل کامپوننت بود، نه در آن فایل‌ها. `screens.jsx` — همان سوئیتی که
  کارش «هر صفحهٔ روت‌شده را mount کن» است — `/launch` را در فهرست نداشت.
  حالا صفحهٔ لانچ در فهرست `screens.jsx` هست و یک سوئیت اختصاصی هم دارد:
  `test/launch-crash-hunt-probe.jsx` ۲۶ ادعا (mount واقعی صفحه با همهٔ
  providerها، در StrictMode و داخل `RouteBoundary` مثل خود اپ) که فقط
  «خطا نداد» را نمی‌سنجد بلکه می‌گوید ویزارد واقعاً **رسم شده** (هیرو، پنج
  قدم، گرید زنجیره‌ها، بخش شبکه) و بعد ده شکل خصمانهٔ `/api/launch/config`
  را هم می‌گذراند. با `npm run test:launch-page` و همچنین داخل `npm test`
  اجرا می‌شود. این گارد sabotage-verified است: با برگرداندن باگ، ۲۴ ادعای
  آن قرمز می‌شود.

# ۲۰۲۶-۰۹-۱۴ — لانچ سولانا زنده شد (Raydium LaunchLab)

- **سولانا دیگر «به‌زودی» نیست** — کارت Solana در قدم شبکهٔ صفحهٔ لانچ حالا
  به فلوی واقعی منحنی پیوندی می‌رسد: ساخت مینت SPL + متادیتای Metaplex و باز
  کردن استخر **Raydium LaunchLab** (`initializeV2`) با خرید اول اختیاری در
  همان فلومی. pump.fun و «فقط توکن» رد شدند؛ مهاجرت فقط CPMM است.
- **قوانین ثابت، گفته‌شده از قبل** — اعشار همیشه ۶؛ سازنده از قبل هیچ توکنی
  نمی‌گیرد (سهم او همان خرید اول است)؛ نماد حداکثر ۱۰ کاراکتر؛ لینک متادیتا
  اجباری (خودکار بدون سرور، یا لینک خودتان)؛ اختیار مینت هنگام ساخت در استخر
  قفل می‌شود. کران‌های منحنی (حداقل عرضه/فروش/جذب) از **پیکربندی زندهٔ
  آن‌چین** خوانده می‌شوند، نه از عددهای ثابت.
- **امضای امن مثل EVM** — بایت‌های تراکنش قطعی‌اند؛ هر تراکنش قبل از درخواست
  امضا شبیه‌سازی می‌شود و شبیه‌سازیِ ناموفق یعنی اصلاً از شما امضا خواسته
  نمی‌شود؛ نرخ خرید اول لحظاتی قبل از پرامپت کیف از وضعیت زندهٔ استخر
  تازه‌سازی می‌شود؛ و «LIVE» فقط وقتی ادعا می‌شود که استخر، مینت، خزانه‌ها و
  متادیتا با طرح تطابق داشته باشند. اگر ساخت موفق ولی خرید اول شکست بخورد،
  دکمهٔ «تلاش دوباره برای خرید اول» همان خرید را با نرخ تازه تکرار می‌کند.
- **اثبات آفلاین** — پروب سولانا آدرس PDA پیکربندی را با عدد طلایی چک
  می‌کند، هر ۵ discriminator را از sha256 از نو می‌سازد، ریاضی منحنی را با
  جواب‌های معلوم (از جمله «خرید کل منحنی دقیقاً برابر هدف جذب است») قفل
  می‌کند، بایت‌های initializeV2/buyExactIn را مستقل از سازنده دیکد می‌کند و
  مونتاژ دو امضایی (مینت + کیف) را تا عبور از serialize/deserialize
  راستی‌آزمایی می‌کند — بدون حتی یک تماس مین‌نت.
- **تعمیرهای جانبی سوئیت تست (همه از قبل قرمز بودند، بدون تغییر رفتار
  محصول)** — دو ایمپورت بدون پسوند در `lifi.js` که پروب شبیه‌سازی را در
  Node می‌شکست؛ دو ادعای stale نسخهٔ v5 سرویس‌ورکر (سورس از قبل v6 است)؛
  به‌روزرسانی ادعاهای Solana در پروب لانچ به حقیقت shipped؛ حذف مثبت‌کاذب
  «پارسی هاردکدشده» و «Notification محافظت‌نشده» که کامنت‌ها را کد حساب
  می‌کردند (و در همین مسیر یک خوانش واقعاً غیرکانونیکال به گارد استاندارد
  برگشت)؛ به‌روزرسانی تست URL سواپ بربران که فرض «استیبل لیست‌نشده» آن با
  curator شدن USDC منقضی شده بود؛ و re-baseline صادقانهٔ رچت باندل اول
  (۱۳۱۰KB در HEAD ‏قبل از سولانا، ۱۳۲۱KB با سولانا که فقط ۱۱KB لوکیل است و
  فلوی lazy هیچ‌چیز به first-paint اضافه نمی‌کند).

# ۲۰۲۶-۰۹-۱۴ — تعمیر لانچ Optimism و Polygon + تعمیر باگ‌های صفحهٔ لانچ + مدرن‌سازی

- **Optimism واقعاً کار می‌کند** — رجیستری قبلی یک فکتوری SushiSwap از زنجیرهٔ
  دیگر را کپی کرده بود (`0xFbc1…B303C`)؛ اثبات CREATE2 نشان داد آدرس جفت
  لنگر آن روی Optimism هرگز کد یا تراکنشی نداشته، پس `verifyDex` همیشه
  `ANCHOR_PAIR_MISSING` می‌داد و این زنجیره برای همیشه بی‌صدا مسدود بود.
  حالا لانچ Optimism روی **Uniswap V2 رسمی** است (فکتوری `0x0c3c…74Bf` و
  روتر `0x4A7b…62c2` از صفحهٔ v2-deployments خود Uniswap) با لنگر WETH/USDC
  که CREATE2 آن دقیقاً به جفت زندهٔ `0x4C43…7815` (دارای WETH+USDC) می‌رسد.
  پروب لانچ ثابت جدید را قفل می‌کند و ثابت ردشده را ممنوع.
- **Polygon واقعاً کار می‌کند** — ثابت‌های QuickSwap سالم بودند (جفت لنگر
  WMATIC/USDT با بیش از ۱۱۱ هزار دلار نقدینگی زنده است)؛ مشکل فقط
  rate-limit تهاجمی `polygon-rpc.com` بود. حالا Polygon و Optimism هر دو
  پنج RPC دارند (publicnode/drpc/llamarpc اول، قدیمی‌ها به‌عنوان fallback)
  و بنر راستی‌آزمایی دکمهٔ «تلاش مجدد» دارد تا RPC ناپایدار شبیه زنجیرهٔ
  خراب به نظر نرسد.
- **قیمت اولیه (Initial price) کار می‌کند** — فیلد قیمت `disabled` بود تا
  وقتی حالت `price` شود، ولی تنها کدی که حالت را عوض می‌کرد `onChange` خود
  همان فیلد غیرفعال بود: دست‌نیافتنی. حالا یک کنترل سگمنتی صریح
  (با مقدار / با قیمت) حالت را عوض می‌کند و هر حالت، فیلد دیگر را
  خودکار حساب می‌کند.
- **سوییچ‌های قوانین خاموش هم می‌شوند** — `Switch` ایونت کلیک (همیشه
  truthy) را به‌جای بولین جدید پاس می‌داد، پس هر قانونی یک‌بار روشن می‌شد و
  دیگر خاموش نمی‌شد. قرارداد اصلاح شد (`onChange(!on, event)`) و صفحهٔ
  لانچ هم صریح toggle می‌کند؛ همهٔ مصرف‌کننده‌های دیگر آرگلس بودند و
  unaffected ماندند.
- **هشدار موجودی درست نوشته شد و داخل باکس است** — عنوان قبلاً قالب خام
  (`موجودی {{sym}} کافی نیست`) را نشان می‌داد چون interpolation پاس داده
  نشده بود؛ حالا مقدار واقعی دارد. هشدارهای حیاتی پول (کمبود موجودی،
  نداشتن گس، نقدینگی کم‌عمق) دیگر پشت باکس جمع‌شونده پنهان نیستند و در
  باکس هشدار همیشه‌باز با آیکون نمایش داده می‌شوند. کمبود شناخته‌شدهٔ
  موجودی/گس دکمهٔ لانچ را هم مسدود می‌کند تا کاربر گس روی ریورت هدر ندهد.
- **دقت موجودی با parseUnits** — مقایسهٔ قبلی با `Number() * 10**decimals`
  برای کووت ۱۸ اعشاری هم می‌توانست لانچ funded را اشتباه مسدود کند و هم
  لانچ unfunded را اشتباه عبور دهد. حالا محاسبهٔ اعشاری دقیق است.
- **پلن مرور stale نمی‌ماند** — برگشتن و عوض‌کردن مقادیر/توکن/اسلیپیج،
  پلن را باطل و در قدم مرور از نو می‌سازد؛ و قطع‌شدن کیف وسط اجرا به‌جای
  فریز، خطای نام‌دار می‌دهد.
- **اتصال کیف در همان‌جا (پاپ‌آپ، بدون رفتن به صفحهٔ والت)** — قدم مرور
  کارت وضعیت کیف دارد؛ اگر کیف وصل نباشد دکمهٔ لانچ شیت اتصال را همان‌جا
  باز می‌کند و همهٔ مقادیر تایپ‌شده حفظ می‌ماند.
- **نام DEX درست** — بنر تأیید و ساب‌تایتل مرور قبلاً نام روتر سواپ را نشان
  می‌دادند (روی Optimism اشتباه می‌گفت Velodrome) یا خالی بودند؛ حالا نام
  واقعی DEX لانچ را نشان می‌دهند.
- **مدرن‌سازی هر دو تم** — نشان گرادیانی شبکه (ChainMark) در کارت‌ها،
  تاریخچه و به‌عنوان لوگوی پیش‌فرض توکن وقتی کاربر لوگو انتخاب نکند؛
  باکس‌های مبلغ مدرن با دکمه‌های ٪عرضه و MAX (فقط برای کووت ERC-20 — مکس
  کردن native گس را صفر می‌کند)؛ اسلایدر و چیپ‌های اسلیپیج؛ ایموجی‌ها با
  آیکون‌های SVG جایگزین شدند؛ فاصلهٔ دکمه/باکس با wrapper یکدست شد.
- **Solana** — عمداً همچنان «به‌زودی» است: مسیر امضا + استخر Raydium +
  راستی‌آزمایی زنده تست‌نشده‌اند و مطابق درپوش صداقت، چیزی که اثبات نشده
  باشد ship نمی‌شود — وگرنه دقیقاً همان «هدررفتن پول کاربر» رخ می‌دهد که
  نباید. کارت آن مدرن و صادقانه شد (تعداد بخش‌های باقی‌مانده + استیتمنت).

# ۲۰۲۶-۰۹-۱۳ (۲) — لانچ توکن بدون فکتوری + سه شبکهٔ جدید روی DEX تأییدشده + بازطراحی صفحهٔ لانچ

حالت پیش‌فرض لانچ دیگر به هیچ قرارداد FBT وابسته نیست: توکن با بایت‌کد آمادهٔ
خودش، مستقیم از کیف کاربر ساخته می‌شود (`to = null`) و آدرس پیش از امضا
پیش‌بینی می‌شود. فکتوری فقط وقتی وارد می‌شود که یک اپراتور آدرس آن را با
`FBTLAUNCH_FACTORY_<chainId>` پین کند.

- **دیپلوی مستقیم (پیش‌فرض نسخهٔ ۱)** — `buildDirectTokenCreateTx`: داده = بایت‌کد
  ساخت + `abi.encode(name, symbol, decimals, initialSupply, creator, capabilities)`;
  آدرس = ۱۶۰ بیت آخر `keccak256(rlp([deployer, nonce]))` و پیش از امضا نمایش داده
  می‌شود. بعد از تأیید: `status == 1` و `to == null` و سپس مقایسهٔ **بایت‌به‌بایت**
  کد آن آدرس با `deployedBytecode` — هر مغایرت یک خطای نام‌دار است
  (`DIRECT_CODE_MISMATCH`, `DIRECT_ADDRESS_MISMATCH`, …) و لانچ را متوقف می‌کند.
  در این حالت هیچ رویداد `TokenCreated` خوانده نمی‌شود (فکتوری‌ای در تراکنش نیست).
- **گزارش صادقانهٔ حالت** — `GET /api/launch/config` برای هر شبکه `mode`
  (`direct` | `factory`) و `tokenDeployReady` برمی‌گرداند؛ هر زنجیره فقط وقتی در
  لیست است که DEX آن کامل پین شده باشد، و پیش از هر امضا
  `verifyDex` چهار چیز را زنده ثابت می‌کند: کد فکتوری، `getPair(anchor)`،
  `pair.factory()` و `router.factory()`/`router.WETH()`. هر ناسازگاری = مسدود.
- **Optimism (10) و Avalanche (43114) اضافه شدند** — هر دو روی SushiSwap V2 با
  روتر/رپد مخصوص خودشان (چون روتر سواپ این زنجیره‌ها Velodrome/Trader Joe است و
  `addLiquidity` سازگار با V2 نیست). آدرس‌ها از رجیستری رسمی Sushi + اثبات
  رمزنگاری‌شدهٔ CREATE2 روی استخر واقعی گرفته شده‌اند؛ آدرس دست‌نویس غلط
  فکتوری Arbitrum هم با مقدار استاندارد و checksum معتبر جایگزین شد.
- **صفحهٔ لانچ** — هدر شیشه‌ای با موشک SVG (شنا + هالهٔ طلایی، زیر
  `prefers-reduced-motion` کامل خاموش)، نشان حالت (مستقیم/فکتوری)، نوار
  زنجیره‌ها با حالت هر شبکه، ردیف آدرس پیش‌بینی‌شده در کارت مرور، و دو باکس
  بازشو («چطور کار می‌کند» و «هشدارها») با متن کامل en+fa. گرید زنجیره‌ها
  auto-fill است و در ۳۹۰ پیکسل سرریز نمی‌کند.
- **Solana — درپوش صداقت** — ماژول وضعیت (`lib/launch/solana/status.js`) می‌گوید چه
  چیزی آماده و چه چیزی ناتمام است و UI فقط از همان می‌خواند؛ برنامهٔ توکن SPL
  (میدان توکن، `InitializeMint`، حساب وابسته، MintTo و لغو اختیار) ساخته و با
  دیکودر مستقل و mock provider تست شده، اما مسیر امضا و ساخت استخر Raydium
  عمداً پیاده نشده و آداپتور Raydium به‌جای حدس‌زدن بایت، امتناع نام‌دار
  می‌کند — پس Solana همان `COMING_SOON` می‌ماند.
- **تست‌ها** — `launch-probe` به ۱۲۷ اسی‌شِن رسید (بردارهای CREATE غیرسن ۰/۱/۲۵۵/۲۵۶،
  برابری دیکود calldata، ده‌ها خطای نام‌دار verify، معناشناسی mode، پوشش i18n و
  قول‌های CSS) و `launch-solana-probe` با mock provider (بدون هیچ تماس mainnet)
  اضافه شد.
...
- **فاز ۲۱ — Cross-Chain Route Intelligence**: `server/fios/routeIntelligence.js` — هر برنامه پل/سواپ بین‌زنجیره‌ای حالا مسیرهای کاندید (مستقیم / چندپایه / از شبکه واسط) را روی ابعاد واقعی رتبه‌بندی می‌کند: هزینه کل مسیر (گاز + کارمزد پل/پروتکل)، عمق نقدینگی و اثر قیمت، زمان تقریبی، نرخ موفقیت تاریخی مسیر، ریسک پل. خروجی: رتبه‌بندی + امتیاز هر بُعد + چرا این مسیر برنده شد و چرا بقیه رد شدند (HOP_LIMIT_EXCEEDED / UNSUPPORTED_CHAIN با دلیل). داده نبود → بُعد UNKNOWN و امتیاز کل محافظه‌کارانه (فقط وزنِ بُعدهای خوانده‌شده) — هرگز عدد جعلی. سیم‌کشی به EXECUTE_BRIDGE (بسته به رکورد تصمیم `bestRoute` با `dimensions[]`)، whyEngine (whyNetwork: «route intelligence ranked N candidate route(s)» + «total route cost X USD» + «liquidity depth Y USD» — همان «این شبکه چون گاز کمتر و نقدینگی عمیق‌تر») و روت‌های عمیق `/api/ai/deep/route-intelligence/*`. پروب ۱۸/۱۸: مسیر ارزانی می‌بازد (امتیاز ۱.۰ در برابر ۰.۴)؛ مسیری که نقدینگی‌اش خالی شده — حتی ارزان‌تر — آخر رتبه می‌شود (اثر قیمت ۱۲۵٪)؛ مسیر سایه با ۴ بُعد UNKNOWN هرگز برنده نمی‌شود.
- **فاز ۲۱۵ — RWA/سهام/فارکس/کالایی در چرخه تصمیم**: `server/fios/traditionalAssets.js` — همین قرارداد فرصت کریپتو، بدون هیچ if/else موازی: کشف از intelligence جهانی (etf/funds = NO_FEED/UNREADABLE، هرگز فرض‌شده)، امتیازدهی، تخصیص چندکلاسی (پیش‌تنظیم محافظه‌کارانه/متعادل/رشد/تهاجمی؛ تنها اسلیو قابل‌اجرا = stable)، فرصت‌های سنتی در agentCouncil، و بلوک `multiClass` روی سناریوهای هدف (سرمایه از خالص‌ارزش واقعی یا صریح، وگرنه NO_CAPITAL_READ — هرگز پیش‌فرض). اجرا فقط از طریق رجیستری broker/off-ramp provider (register/unregister/list): بدون provider پیکربندی‌شده، رد صادقانه فارسی «این کلاس دارایی فقط تحلیل می‌شود، اجرا ندارد — broker/off-ramp provider برای این کلاس configure نشده است» — هرگز اجرای شبیه‌سازی‌شده. با provider، فقط hand-off امضانشده‌ی خودِ provider (برگه بعدی، نه ما). `instrumentOf` در `src/lib/central/intent.js` هنوز فقط طبقه‌بندِ query است. پروب ۲۴/۲۴.
- **فاز ۲۱۶ — آداپتورهای وام واقعاً ساخته شدند و روشن شدند**: compound-v3 (Comet Base/USDC: supply/withdraw روی `compoundV3Base` و borrow/repay/health-factor روی `compoundV3Lending` — ماژول جدا، تا سطح Farm فقط supply/withdraw بماند و پینِ wiring آن را نگه دارد) ، morpho (Morpho Blue Base USDC/cbBTC: supply/withdraw/borrow/repay روی `morphoBlueBase`؛ market id = keccak256 پارامترهای بسته‌شده) و solana-lending (آداپتور registry استخر) — همه `enabled: true`، `networkConfig.js` همگام با شبکه‌های مؤثر. درگاه‌های امنیتی BAD_ADDRESS / UNSUPPORTED_CHAIN / POOL_NOT_ALLOWED / TOKEN_NOT_ALLOWED دست‌نخورده — آداپتورها پشت درگاه هستند، درگاه را باز نکرده‌اند. خطای شبکه → کد خطای صادقانه (ADAPTER_MODULE_UNAVAILABLE و…)، هرگز موفقیت جعلی. ۱۱۱ چک vitest + پروب ۳۰/۳۰.
- **بگ‌های واقعی که این راند گرفت**: (۱) `totalCostUsd` صریح در نقل‌قول مسیر، بُعد هزینه را UNKNOWN می‌گذاشت (فقط پای‌های کارمزی می‌شمردند) — یعنی «مسیر ارزانی می‌بازد» فقط با tie-break تصادفی رد می‌شد؛ حالا هزینه با جزئیات «provider-quoted total path cost X USD» شناخته‌شده و رتبه‌بندی قطعی شد. (۲) `bestRoute` ذخیره‌شده در رکورد تصمیم `dimensions[]` نداشت و whyEngine چیزی برای نقل‌قول نداشت — اضافه شد. (۳) «تغییر ۲۴ ساعته مشاهده‌شده» در تخصیص چندکلاسی با هر ابزار بعدیِ همان کلاس بازنویسی می‌شد (آخری-برنده، دلخواه) — حالا اولین گزارش‌شده در ترتیب feed برنده است و `observedSymbol` مالکیت عدد را مشخص می‌کند. (۴) مستندات Morpho نسخه‌ی سیگناتور جدیدتر (receiver+onBehalf) را نشان می‌دهد، اما استقرار Base (0xBBBB) سیگناتورهای اصلی ۵-آرگومان دارد — آداپتور روی همان سیگناتورهای پین‌شده‌ی استقرار کار می‌کند. (۵) پلن بازپرداخت Comet بدون مرحله‌ی approve ساخته می‌شد، در حالی که `repay` Comet تتر را با `transferFrom` از کیف پول کاربر **کش می‌آورد** — یعنی هر بازپرداخت بدون اجازه‌ی قبلی بعد از امضا با `TransferInFailed()` برمی‌گشت. پلن حالا وقتی اجازه کافی نیست، مرحله‌ی approve به‌اندازه‌ی دقیق مبلغ بازپرداخت می‌سازد (تست رگرسیون: `['approve','repay']`).
- **مشخصات تا فاز ۲۱۶، ۱۹۶ فاز منتشرشده، ۱۶۶ فاز محصولی لایو** — سه ردیف فاز جدید (۲۱۴/۲۱۵/۲۱۶) با `testsPresent: true`، `specificationImplementedThrough: 216`، `phaseCount: 196`؛ پروب‌های phase-status (۱۷/۱۷) و durable-evidence همگام شدند و سه پروب جدید در حلقه‌ی FIِ `test/run.mjs` سیم‌کشی شدند. `npm test` کامل: All suites passed.
...
# Unreleased — نهایی‌سازی اعتماد: روتر به پنل‌ها وصل شد، سقف‌ها برداشته شد، ارورهای شبکه درست شد، پنل‌ها به تب «داخل اپ» آمدند

**چهار دستور کاربر، هر چهار تمام و تست‌شده:** «درستش کن و نهایی بشه» (سیم‌کشی
روتر به هر ۵ پنل، با افشای کارمزد قبل از امضا)، «برای هر پنج شبکه هم محدودیت
را کلا بردار» (حذف کامل سقف‌ها)، «بعضی شبکه‌ها هنوز ارور LIDO_NETWORK_UNREADABLE
… درستش کن» (RPCهای بیشتر + retry)، و «هر ۵ شبکه که زیر صفحه فارم هست را بیار
داخل تب داخل اپ».

- **حذف کامل سقف‌ها (uncapped).** `features.js` envCap و ۱۰ خروجی سقف حذف شدند؛
  هر ۵ آداپتور (Aave Base/Arb، Compound، Morpho، Lido) بدون سقف per-tx/total
  هستند — تنها محدودیت‌های باقی‌مانده مالِ خود پروتکل‌هاست (سقف تأمین ذخیرهٔ
  Aave، موجودی کیف پول، کف گس) و دکمهٔ Max از موجودی کیف پول پر می‌شود.
  `farm-rollout-policy` حالا «(uncapped)» چاپ می‌کند و مقادیر `VITE_*_MAX_*`
  قدیمی کلاً نادیده گرفته می‌شوند. تست: farm 261/261، gate 23/23، wiring exit 0.
- **رفع ارورهای شبکه (`*_NETWORK_UNREADABLE`).** ریشهٔ ماجرا rate-limit بود، نه
  قطعی: فهرست RPC عمومی اتریوم فقط ۲ endpoint داشت. حالا Ethereum/Base/Arbitrum
  هرکدام ۴ endpoint دارند (publicnode + drpc جلوتر)، `assertProviderChain`
  تا ۳ بار با backoff کوتاه `getNetwork` را می‌خواند (wrong-chain هرگز retry
  نمی‌شود)، و refresh هر ۵ پنل با `withRpcRetry` (فقط خواندن‌ها — هرگز ارسال/امضا)
  مسیریابی مجدد می‌شود. تست: rpc-retry + guards ۱۲ مورد جدید.
- **جابه‌جایی پنل‌ها به تب «داخل اپ».** پنل‌های اجرای ۵ شبکه از پایینِ همهٔ
  تب‌ها به داخل تب «داخل اپ» منتقل شدند و فقط همان‌جا رندر می‌شوند — برای هر
  بازدیدکننده، وصل یا غیر وصل. تست hub به تب inapp گره خورد + عدم حضور در
  تب‌های دیگر assert می‌شود.
- **سیم‌کشی SplitRouter به هر ۵ پنل (پیش از هر امضا افشا می‌شود).** پلن supply
  (و فقط supply — برداشت/خروج هرگز کارمزد ندارد) وقتی روتری برای آن شبکه
  تنظیم شده باشد از `routeSupplyPlan` عبور می‌کند: approve به روتر بازنویسی
  (یا تزریق) می‌شود، `checks.splitRouter` کارمزد/خالص را ثبت می‌کند و شیت قبل
  از دکمهٔ امضا «کارمزد واریز: X از Y (Z٪) … برداشت همیشه بدون کارمزد است» را
  نشان می‌دهد. اثبات receipt دو لایه شد: رویداد `Routed` (split صادقانه) +
  رویداد خودِ پروتکل که باید مالک را با مبلغ **خالص** بدهد. revoke هم وقتی
  روتر فعال است اجازهٔ روتر را لغو می‌کند. fail-open: بدون آدرس روتر، پلن
  ذره‌ای تغییر نمی‌کند (همان مرجع). تست: split-router 26 + receiptهای روترشده
  در هر ۴ آداپتور.
