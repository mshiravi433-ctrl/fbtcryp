...
export default {
  ...
  'about.faq.a5': {
    zh: '通过 FBT AI 网关和编排器，平台可以使用已配置的提供方（如 Grok 或 OpenRouter）以及内部智能系统来分析市场、链上和投资组合数据。它负责辅助和解释——不保证结果。',
    hi: 'FBT AI गेटवे और ऑर्केस्ट्रेटर के ज़रिए प्लेटफ़ॉर्म कॉन्फ़िगर किए गए प्रोवाइडर (जैसे Grok या OpenRouter) और आंतरिक इंटेलिजेंस सिस्टम से बाज़ार, ऑन-चेन और पोर्टफ़ोलियो डेटा का विश्लेषण कर सकता है। यह सहायता करता है और समझाता है — नतीजों की गारंटी नहीं देता।',
    es: 'A través del FBT AI Gateway y el orquestador, la plataforma puede usar proveedores configurados (como Grok u OpenRouter) y sistemas de inteligencia internos para analizar datos de mercado, en cadena y de cartera. Asiste y explica; no garantiza resultados.',
    fr: 'Via la passerelle FBT AI et l\u2019orchestrateur, la plateforme peut s\u2019appuyer sur des fournisseurs configurés (comme Grok ou OpenRouter) et des systèmes d\u2019intelligence internes pour analyser les données de marché, on-chain et de portefeuille. Elle assiste et explique — elle ne garantit aucun résultat.',
    ru: 'Через шлюз FBT AI и оркестратор платформа может использовать настроенных провайдеров (например, Grok или OpenRouter) и внутренние системы аналитики для анализа рыночных, ончейн- и портфельных данных. ИИ помогает и объясняет — но не гарантирует результат.',
    tr: 'FBT AI Gateway ve orkestratör üzerinden platform, yapılandırılmış sağlayıcıları (Grok veya OpenRouter gibi) ve dahili zekâ sistemlerini kullanarak piyasa, zincir üstü ve portföy verilerini analiz edebilir. Yardımcı olur ve açıklar; sonuç garanti etmez.',
    ur: 'FBT AI گیٹ وے اور آرکسٹریٹر کے ذریعے پلیٹ فارم کنفیگر شدہ فراہم کنندگان (جیسے Grok یا OpenRouter) اور اندرونی انٹیلیجنس سسٹمز سے مارکیٹ، آن چین اور پورٹ فولیو ڈیٹا کا تجزیہ کر سکتا ہے۔ یہ مدد اور وضاحت کرتا ہے — نتائج کی ضمانت نہیں دیتا۔',
    id: 'Melalui FBT AI Gateway dan orkestrator, platform dapat memakai penyedia yang dikonfigurasi (seperti Grok atau OpenRouter) dan sistem kecerdasan internal untuk menganalisis data pasar, on-chain, dan portofolio. Ia membantu dan menjelaskan, bukan menjamin hasil.',
    pt: 'Através do FBT AI Gateway e do orquestrador, a plataforma pode usar fornecedores configurados (como Grok ou OpenRouter) e sistemas de inteligência internos para analisar dados de mercado, on-chain e de carteira. Ela ajuda e explica; não garante resultados.'
  },
