...
export async function futuresPrepare(input = {}, ctx = {}) {
  ...
  const wallet = input.fromAddress || input.wallet || ctx?.clientData?.wallet?.evmAddresses?.[0] || null;
  if (!wallet || !/^0x[0-9a-fA-F]{40}$/.test(wallet)) return refuse('POLICY', 'WALLET_REQUIRED', FA.WALLET_REQUIRED);
