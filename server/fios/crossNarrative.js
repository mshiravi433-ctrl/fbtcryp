...
const SYSTEM_FA = [
  'شما یک تحلیل‌گر ارشد اقتصاد کلان هستید و باید یک پاراگراف تحلیل جامع از وضعیت فعلی بازارها بنویسید.',
  'قوانین غیرقابل‌مذاکره:',
  '۱) هرگز متن محلی (local_analysis) را کپی نکنید. یک تحلیل کاملا جدید و حرفه‌ای ارائه دهید که اقتصاد کلان و ارزهای دیجیتال را ترکیب کند.',
  '۲) کاملا به زبان فارسی سلیس، حرفه‌ای و روان بنویسید (از هیچ کلمه انگلیسی استفاده نکنید).',
  '۳) فقط از اعداد ارائه شده در «facts» استفاده کنید؛ هیچ عددی از خودتان نسازید.',
  '۴) هیچ توصیه خرید/فروش ندهید.',
  '۵) حداکثر ۱۰۰ کلمه در یک پاراگراف یکپارچه.'
].join(' ');
const SYSTEM_EN = [
    ...
    try {
      const routed = await Promise.race([
        routedChat({
          ...
          user: `${persian ? 'حقایق بازار (فقط از این داده‌ها برای تحلیل اقتصادی و دارایی‌ها استفاده کن و متن قبلی را کپی نکن):' : 'This pass\u2019s facts (only these):'}\n${factsOf(analysis, { persian })}\n\n${persian ? 'یک پاراگراف تحلیل جامع، حرفه‌ای و متفاوت به زبان فارسی بنویس که شامل اقتصاد کلان و ارزهای دیجیتال باشد.' : 'Write the one comprehensive English paragraph.'}`,
          temperature: 0.2,
          maxTokens: 420
        }),
