/**
 * FBT INTENT AI — specification phase registry (leaf module).
 *
 * The authoritative list of specification phases (10–216) with their
 * implementation sources and probe files. Kept dependency-free (node builtins
 * only) so both the status reporter and the runtime plane-input collector can
...
export const sourceExists = (file) => existsSync(resolve(REPOSITORY_ROOT, file));
const BASE_SPEC_PHASES = Object.freeze([
  ...
  { phase: 11, id: 'strategy-competition-and-simulation', title: 'Strategy Generation و Competition و Simulation', implementation: 'implemented', source: ['src/lib/intent-ai/strategyCompetition.js', 'server/fios/routeSimulator.js', 'server/fios/smartMoneyIntel.js', 'server/fios/competition.js'], tests: ['test/intent-ai/phase11-strategy-competition-probe.mjs', 'test/intent-ai/phase11-live-strategy-probe.mjs'], requiredEvidence: ['route-simulation-provider', 'observed-evidence'] },
...
/* Phases 152 (flash-liquidity readiness) and 153 (cross-chain live pricing)
   carry dedicated probes beyond the shared autonomy recovery probe. */
const AUTONOMY_EXTRA_TESTS = Object.freeze({
  152: ['test/intent-ai/phase152-flash-liquidity-probe.mjs'],
  153: ['test/intent-ai/phase153-cross-chain-live-probe.mjs']
});
...
const AUTONOMY_SPEC_PHASES = Object.freeze(AUTONOMY_PHASE_TITLES.map((title, index) => ({
  ...
  tests: ['test/intent-ai/phase151-autonomous-recovery-probe.mjs', ...(AUTONOMY_EXTRA_TESTS[151 + index] || [])],
  requiredEvidence: []
})));
...
/* Phases 201–209 + 211 — the Intent OS intelligence upgrades. Each row names
   the real module plus the probe files that exercise it (logic and mounted
   panel). Like every phase ≥ 51, the launch-wide evidence gate decides
   operational/live — these rows never paint themselves live. */
const INTENT_OS_UPGRADE_PHASES = Object.freeze([
  {
    phase: 201,
    id: 'taught-memory',
    title: 'Taught Memory (user-taught facts: teach/recall/forget)',
    implementation: 'implemented',
    source: ['src/lib/intent-ai/taughtMemory.js'],
    tests: [
      'test/intent-ai/phase201-207-upgrade-probe.mjs',
      'test/intent-ai/phase201-ai-panel-upgrade-probe.jsx'
    ],
    requiredEvidence: []
  },
  {
    phase: 202,
    id: 'external-agent-voice',
    title: 'External Agent Voice (persona routing, honest no-data)',
    implementation: 'implemented',
    source: ['src/lib/intent-ai/externalAgentVoice.js'],
    tests: ['test/intent-ai/phase201-207-upgrade-probe.mjs'],
    requiredEvidence: []
  },
  {
    phase: 203,
    id: 'first-party-agent-catalog',
    title: 'First-party Agent Catalog (market analyst + risk auditor)',
    implementation: 'implemented',
  ...
  {
    phase: 204,
    id: 'upgrade4-intent-understanding',
    title: 'Upgrade 4 — Intent Understanding (understanding + confidence + session)',
    implementation: 'implemented',
    source: [
      'src/lib/intent-ai/os/intentUnderstandingEngine.js',
      'src/lib/intent-ai/os/intentUnderstanding.js',
      'src/lib/intent-ai/os/confidenceEngine.js',
      'src/lib/intent-ai/os/humanResponse.js',
      'src/lib/intent-ai/os/intentSession.js',
      'server/aiLearning.js',
      'server/aiConsensus.js'
    ],
    tests: ['test/intent-ai/phase204-upgrade4-intent-intelligence-probe.mjs'],
    requiredEvidence: []
  },
  {
    phase: 205,
    id: 'upgrade5-collaborative-intelligence',
    title: 'Upgrade 5 — Collaborative Intelligence (multi-agent sessions)',
    implementation: 'implemented',
    source: [
      'server/aiCollaboration.js',
      'src/lib/intent-ai/collaborationSession.js'
    ],
    tests: ['test/intent-ai/upgrade5-collaborative-intelligence-probe.mjs'],
    requiredEvidence: []
  },
  {
    phase: 206,
    id: 'upgrade6-stateful-os',
    title: 'Upgrade 6 — Stateful OS (conversation state + slot filling)',
    implementation: 'implemented',
    source: [
      'src/lib/intent-ai/os/upgrade6/index.js',
      'src/lib/intent-ai/os/upgrade6/conversationState.js',
      'src/lib/intent-ai/os/upgrade6/slotFillingEngine.js'
    ],
    tests: ['test/intent-ai/upgrade6-stateful-os-probe.mjs'],
    requiredEvidence: []
  },
  {
    phase: 207,
    id: 'upgrade7-deep-intelligence',
    title: 'Upgrade 7 — Deep Intent (graph + planner + agent mesh)',
    implementation: 'implemented',
    source: [
      'src/lib/intent-ai/os/upgrade7/index.js',
      'src/lib/intent-ai/os/upgrade7/deepIntent.js',
      'src/lib/intent-ai/os/upgrade7/intentGraph.js',
      'src/lib/intent-ai/os/upgrade7/planner.js'
    ],
    tests: ['test/intent-ai/upgrade7-intelligence-probe.mjs'],
    requiredEvidence: []
  },
  {
    phase: 208,
    id: 'upgrade8-orchestration',
    title: 'Upgrade 8 — Orchestration (controller + goals + questions)',
    implementation: 'implemented',
    source: [
  ...
  },
  {
    phase: 209,
    id: 'command-deck',
    title: 'Command Deck (AI Control panel + automation plans)',
    implementation: 'implemented',
    source: [
      'src/components/IntentAIPanel.jsx',
      'src/lib/intent-ai/commandCenter.js'
    ],
    tests: ['test/intent-ai/phase209-command-deck-probe.jsx'],
    requiredEvidence: []
  },
  {
    phase: 211,
    id: 'global-intelligence',
    title: 'Global Intelligence (macro intel + cross-asset + briefing)',
    implementation: 'implemented',
    source: [
      'server/fios/globalIntel.js',
      'server/fios/crossAsset.js',
      'server/fios/briefing.js',
      'src/components/ai/AiGlobalIntelligence.jsx'
    ],
    tests: [
      'test/intent-ai/phase211-global-intelligence-probe.mjs',
      'test/intent-ai/phase211-routes-inventory-probe.mjs',
      'test/intent-ai/phase211-ai-global-panel-probe.jsx'
    ],
    requiredEvidence: []
  }
]);
...
export const SPEC_PHASES = Object.freeze([...BASE_SPEC_PHASES, ...LATER_SPEC_PHASES, ...AUTONOMY_SPEC_PHASES, ...INTENT_OS_UPGRADE_PHASES, ...FINANCIAL_INTELLIGENCE_PHASES]);

/**
 * True when every specification row has its implementation sources AND its
 * probe files present on disk. Used as the program-completeness fact for
 * control plane 50 — a static, verifiable property of this checkout.
 */
export function specImplementationComplete() {
  return SPEC_PHASES.length > 0 && SPEC_PHASES.every(
    (phase) => phase.source.every(sourceExists) && phase.tests.every(sourceExists)
  );
}
