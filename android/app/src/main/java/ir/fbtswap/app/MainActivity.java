...
public class MainActivity extends BridgeActivity {
  ...
  /*
   * ─── THE APP'S OWN NOTIFICATION TONE ──────────────────────────────────────
   *
   * «نوتیفیکیشن اپلیکیشن وقتی میاد روی صفحه اندروید … یک آهنگ نوتیفیکیشن بزنه
   * که خاص باشد» — when a notification slides down the screen it must announce
   * itself with OUR sound, not with whatever chime the device ships.
   *
   * The tone lives at res/raw/fbt_notification.wav and is generated by
   * scripts/gen-notification-sound.mjs (three struck partials of an A-major
   * triad plus a shimmer an octave up — see that file for the reasoning).
   *
   * HOW ANDROID DECIDES WHAT PLAYS
   *   On API 26+ the sound belongs to the CHANNEL, not to the individual
   *   notification — a `setSound()` on a builder posted to a channel whose
   *   sound is the system default is simply ignored. And a channel's sound
   *   (like its importance) can never be changed once the channel exists, so
   *   channel ids here are versioned: `fbt_alerts_v1`. An install that already
   *   has an older id keeps the old behaviour rather than getting a silent
   *   channel nobody can fix.
   *
   *   FCM resolves the channel it posts to in this order: the message's own
   *   `android.notification.channel_id`, then the manifest meta-data
   *   `com.google.firebase.messaging.default_notification_channel_id`, then
   *   its own `fcm_default_channel`. Both are set (see AndroidManifest.xml and
   *   server/fcm.js), and creating the channels HERE — in onCreate, before any
   *   message can arrive — is what makes the channel exist with OUR settings
   *   instead of being defaulted into existence by the Firebase SDK.
   */
  private static final String CHANNEL_ALERTS = "fbt_alerts_v1";
  private static final String CHANNEL_UPDATES = "fbt_updates_v1";
  private static final String SOUND_NAME = "fbt_notification"; // res/raw/fbt_notification.wav
  ...
  /**
   * Create both notification channels, with our tone on each of them.
   *
   * Called from onCreate on EVERY launch, not once per install: the channel is
   * ours whether the app was installed before or after this code shipped, and
   * re-registering an existing channel is a no-op for the fields Android
   * locks (sound, importance) while still refreshing its name and description.
   *
   * Two channels because two kinds of message arrive:
   *   • ALERTS  (IMPORTANCE_HIGH) — a price hit or an order that filled. High
   *     importance is also what makes the notification drop in from the top of
   *     the screen as a heads-up instead of appearing silently in the shade;
   *   • UPDATES (IMPORTANCE_DEFAULT) — the daily digest. Same tone, no
   *     heads-up: a promotion that interrupts whatever the user is doing is
   *     how notification permission gets revoked.
   */
  private void createNotificationChannels() {
    ...
    try {
      ...
      /* android.resource://<package>/raw/fbt_notification — the Uri form the
         platform resolves for a raw resource. Built from the package name at
         runtime so it cannot drift from the application id. */
      Uri sound = Uri.parse(
        ContentResolver.SCHEME_ANDROID_RESOURCE + "://" + getPackageName() + "/raw/" + SOUND_NAME);
      ...
      alerts.setDescription(getString(R.string.notification_channel_alerts_desc));
      alerts.setSound(sound, audio);
      alerts.enableVibration(true);
      alerts.setVibrationPattern(new long[] { 0, 60, 90, 60 });
      ...
      updates.setDescription(getString(R.string.notification_channel_updates_desc));
      updates.setSound(sound, audio);
      updates.enableVibration(false);
      manager.createNotificationChannel(updates);
    } catch (Exception e) {
      /*
       * A device that refuses to create a channel still has to run. Losing the
       * custom tone costs a little recognisability; throwing in onCreate costs
       * the entire app.
       */
    }
  }
