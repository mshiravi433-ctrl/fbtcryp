...
| **رمز `a09303653064A@`** | هرجایی که استفاده شده، عوضش کن |
...
## deBridge — یک خط درآمد کاملاً رایگان که جا مانده بود

پرسیدی کدام سایت‌ها یادم رفته. **این یکی واقعاً جا مانده بود** و از همهٔ
موارد بخش دو ارزشمندتر است، چون **صفر دلار و صفر ثبت‌نام** دارد.

deBridge (شبکهٔ DLN) پل بین‌شبکه‌ای است که **کلید API نمی‌خواهد** و آدرس
دریافت کارمزد را مستقیم در URL می‌گیرد. امروز با آدرس واقعی خودمان تست زدم —
۱۰ USDC از Base به Arbitrum:

```json
{"type":"AffiliateFee","payload":{"feeAmount":"69972","feeBps":"70"}}
```

**۷۰ bps، یعنی دقیقاً ۰٫۷۰٪، پرداخت مستقیم به `0xaf5CE154...24d6`.**
با `affiliateFeePercent=0.3` هم تست کردم و درست ۳۰ bps برگرداند — یعنی عدد را
هرچه بگذاریم واقعاً اعمال می‌شود، نه اینکه بی‌سروصدا نادیده بگیرد.

مقایسه‌اش با پل فعلی‌مان: LI.FI به ما ۳۰ bps می‌دهد. **deBridge ۷۰ bps** می‌دهد،
یعنی روی همان حجم **۲٫۳ برابر**.

**کاری از تو لازم نیست.** فقط بگو «انجام بده» تا به‌عنوان مسیر دوم پل اضافه‌اش
کنم و هر بار قیمت بهتر را انتخاب کنیم.
...
| ۲ | Alchemy + Groq + Blob + رمز | بالا | رایگان | — (امنیت) |
| ۳ | حذف پروژهٔ `fbtcryp4` | [vercel.com](https://vercel.com/dashboard) | رایگان | — |
| ۴ | **Avantis** کد `fbtswap` | [avantisfi.com](https://www.avantisfi.com) | رایگان | ۵٪ کارمزد |
