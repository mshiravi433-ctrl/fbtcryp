...
import { AAVE_ARB_SUPPLY_OPEN_TO_PUBLIC } from '../../lib/farmRolloutMode';
import {
...
export default function AaveArbUsdcPanel({ pool, initialAmount = null }) {
  ...
  const refresh = useCallback(async () => {
     ...
     * position is read from pinned Arbitrum contracts even when the wallet is
     * on another network, so an imported wallet can still discover its exit.
     */
    setHistory(loadAaveArbHistoryFor(owner));
    // Position discovery uses the pinned Arbitrum read provider regardless of
    // the wallet's active network; the signer guard still requires a switch.
    setBusy('loading');
    try {
      const provider = await wallet.getReadProvider(AAVE_V3_ARBITRUM.chainId);
      const [pos, res, rows] = await Promise.all([
        withRpcRetry(() => getPosition(provider, owner, { history: loadAaveArbHistoryFor(owner) }), { label: 'aave-arb position' }).catch(() => null),
        withRpcRetry(() => getReserveStatus(provider), { label: 'aave-arb reserve' }).catch(() => null),
      ...
      setStatus(res);
      setHistory(rows);
      const bal = await withRpcRetry(() => walletBalanceOf(provider, owner), { label: 'aave-arb wallet' });
      if (!alive.current) return;
      setWalletUsdc(bal);
      const rinfo = await withRpcRetry(
        () => loadSplitRouterInfo(provider, AAVE_V3_ARBITRUM.chainId),
        { label: 'aave-arb router' }
      ).catch(() => null);
      if (!alive.current) return;
      setRouterInfo(rinfo);
  ...
  const maxSupplyUsdc = useMemo(() => {
    /* No platform caps: wallet balance and Aave's own reserve cap only. */
    const limits = [];
  ...
  const buildPlan = useCallback(async (nextAmount, nextMode) => {
    ...
    try {
      ...
      if (nextMode !== 'withdraw' && Array.isArray(built?.steps) && built.steps.length > 0) {
        const rinfo = await loadSplitRouterInfo(provider, AAVE_V3_ARBITRUM.chainId);
        if (rinfo) {
      ...
      try {
        ...
        const outcome = await simulateUnsignedTransaction({
          ...
          allowance: step.kind === 'supply'
            ? { token: AAVE_V3_ARBITRUM.usdc, owner, spender: plan.checks.splitRouter?.address ?? AAVE_V3_ARBITRUM.pool, amountWei: plan.checks.amountWei ?? 0n }
            : undefined
        });
        if (!cancelled) {
   ...
   */
  if (!supplyAllowed && !hasPosition && !knownHere && !AAVE_ARB_SUPPLY_OPEN_TO_PUBLIC) return null;
    ...
    <section className="card card-soft farm-aavearb-panel" aria-label={t('farm.aaveArb.panelTitle')}>
      <div className="row-between" style={{ gap: 10, alignItems: 'flex-start' }}>
        <div style={{ minWidth: 0 }}>
          ...
          <div className="muted" style={{ fontSize: 11.6, margin: '3px 0 0' }}>
            {t('farm.aaveArb.panelSub', { chain: localChainLabel(EVM_CHAINS[AAVE_V3_ARBITRUM.chainId].name, t), symbol: AAVE_V3_ARBITRUM.usdcSymbol })}
          </div>
        </div>
      ...
      {knownHere && (
        <p className="muted" style={{ fontSize: 11.6, margin: '10px 0 0' }}>
          {t('farm.aaveArb.wrongChainNote', { chain: localChainLabel(EVM_CHAINS[AAVE_V3_ARBITRUM.chainId].name, t) })}
        </p>
      )}
      ...
      <div className="farm-actions" style={{ marginTop: 10 }}>
        ...
        {wrongChain && (
          <button className="btn btn-ghost farm-btn" onClick={() => wallet.switchChain(AAVE_V3_ARBITRUM.chainId)}>
            {t('farm.aaveArb.switchChain', { chain: localChainLabel(EVM_CHAINS[AAVE_V3_ARBITRUM.chainId].name, t) })}
          </button>
        )}
          ...
          {plan?.checks?.splitRouter && (
            <p className="notice" style={{ margin: '8px 0 0' }}>
              {t('farm.splitRouter.feeNotice', {
                ...
                symbol: AAVE_V3_ARBITRUM.usdcSymbol
              })}
              <br />
...
async function walletAllowanceOf(provider, owner, spender = null) {
  ...
  try {
    return await c.allowance(owner, spender ?? AAVE_V3_ARBITRUM.pool);
  } catch {
    return 0n;
  }
}
