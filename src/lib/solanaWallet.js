...
/**
 * A deeplink signing answer, turned into the code the UI can translate.
 *
 * The distinction that matters is `IN_WALLET`: on a route that had to navigate
 * this page to the wallet (iOS, and any Android browser without `intent://`)
 * the approval is still happening — the answer simply cannot arrive in a
 * document that is already gone. Reporting that as SIGN_FAILED told users their
 * signature had failed when it had not, which is the worst possible answer to
 * give someone standing in front of their wallet.
 *
 * @param {{ok?:boolean, code?:string|null}} res
 * @param {string} [fallback] the code this call site uses for "did not work"
 */
export function deeplinkSignError(res, fallback = 'SIGN_FAILED') {
  const code = String(res?.code ?? '');
  if (code === 'IN_WALLET') return 'IN_WALLET';
  if (code === 'REJECTED') return 'REJECTED';
  if (code === 'NO_SIGNATURE') return 'NO_SIGNATURE';
  if (code === 'NO_SESSION') return 'NO_WALLET';
  if (code === 'TIMEOUT') return 'TIMEOUT';
  if (code === 'OPEN_FAILED') return 'WALLET_NOT_FOUND';
  return fallback;
}

/** The bare origin of a public URL — what an identity check can be hung off. */
export function identityOrigin(appUrl) {
  const raw = String(
    appUrl
  ...
  try {
    ...
    mod.registerMwa({
      appIdentity: {
        name: 'FBT Swap',
        /*
         * The ORIGIN, not a URL with a path.
         *
         * Phantom verifies an MWA dapp by fetching
         * `<origin>/.well-known/assetlinks.json` and matching our package name
         * and signing certificate against it (docs.phantom.com → «Domain and
         * transaction warnings»). A trailing slash made that request
         * `https://fbtswap.ir//.well-known/…`, and a verification that cannot
         * complete is rendered to the user as «this app's identity could not
         * be verified — it may be impersonating another app». The file itself
         * is generated by `npm run assetlinks` (it needs the release signing
         * fingerprint, so it cannot be invented here).
         */
        uri: identityOrigin(appUrl),
        icon: 'icon-192.png'
      },
...
export async function signSolanaTransaction(base64Tx) {
  ...
  if (!provider && !mwa && deeplinkSession()) {
    ...
    throw new Error(deeplinkSignError(res));
  }
