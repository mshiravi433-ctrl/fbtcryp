/**
 * AAVE V3 (BASE/USDC) LOCAL ACTION LEDGER.
 * ---------------------------------------------------------------------------
 ...
 *      getPosition() in lib/defi/aaveV3Base.js).
 ...
 * timestamp) plus the owner address the user already knows. `recordAaveAction`
 * picks its fields explicitly from a whitelist so a careless caller cannot
 * smuggle a signer or a key into localStorage.
 */

export const AAVE_HISTORY_KEY = 'fbt-aave-base-history-v1';
const MAX_ROWS = 120;
...
export function loadAaveHistory() {
  const rows = readJson(AAVE_HISTORY_KEY, []);
  return Array.isArray(rows) ? rows : [];
}

/** Records for one wallet (case-insensitive), newest first. */
export function loadAaveHistoryFor(owner) {
  const who = String(owner ?? '').toLowerCase();
  if (!who) return [];
  return loadAaveHistory().filter((r) => String(r?.owner ?? '').toLowerCase() === who);
}
...
export function recordAaveAction({
  action = 'supply',
  owner = '',
  amountUsdcWei = null,
  amountUsdc = null,
  ...
  const rows = loadAaveHistory();
  rows.unshift(row);
  writeJson(AAVE_HISTORY_KEY, rows.slice(0, MAX_ROWS));
  return row;
}

/** Patch a record by id — the pending → confirmed / failed transition. */
export function updateAaveAction(id, patch = {}) {
  const rows = loadAaveHistory();
  const target = rows.find((r) => r.id === id);
  if (!target) return null;
  Object.assign(target, sanitize({ ...target, ...patch, id: target.id }));
  writeJson(AAVE_HISTORY_KEY, rows);
  return target;
}

export function confirmAaveAction(id, { txHash = null, blockNumber = null } = {}) {
  return updateAaveAction(id, {
    status: 'confirmed',
    txHash: txHash ?? null,
...
export const cancelAaveAction = (id, error = null) =>
  updateAaveAction(id, { status: 'cancelled', error });
